...
/**
 * madxartwork icons manager.
 *
 * madxartwork icons manager handler class
 *
 * @since 2.4.0
 */
class Icons_Manager {
	...
	private static function get_needs_upgrade_option() {
		return get_option( 'madxartwork_' . self::NEEDS_UPDATE_OPTION, null );
	}
	...
	public function register_styles() {
		...
		foreach ( $config as $type => $icon_type ) {
			...
			if ( ! empty( $icon_type['enqueue'] ) ) {
				foreach ( (array) $icon_type['enqueue'] as $font_css_url ) {
					if ( ! in_array( $font_css_url, array_keys( $shared_styles ) ) ) {
						$style_handle = 'madxartwork-icons-shared-' . count( $shared_styles );
						wp_register_style(
							$style_handle,
							$font_css_url,
							[],
			...
			wp_register_style(
				'madxartwork-icons-' . $icon_type['name'],
				$icon_type['url'],
				$dependencies,
				$icon_type['ver']
			);
	...
	private static function init_tabs() {
		self::$tabs = apply_filters( 'madxartwork/icons_manager/native', [
			'fa-regular' => [
				'name' => 'fa-regular',
				'label' => __( 'Font Awesome - Regular', 'madxartwork' ),
			...
			'fa-solid' => [
				'name' => 'fa-solid',
				'label' => __( 'Font Awesome - Solid', 'madxartwork' ),
			...
			'fa-brands' => [
				'name' => 'fa-brands',
				'label' => __( 'Font Awesome - Brands', 'madxartwork' ),
	...
	public static function get_icon_manager_tabs() {
		...
		$additional_tabs = apply_filters( 'madxartwork/icons_manager/additional_tabs', [] );
	...
	private static function get_fa_asset_url( $filename, $ext_type = 'css', $add_suffix = true ) {
		...
		if ( null === $is_test_mode ) {
			$is_test_mode = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG || defined( 'madxartwork_TESTS' ) && madxartwork_TESTS;
		}
		$url = madxartwork_ASSETS_URL . 'lib/font-awesome/' . $ext_type . '/' . $filename;
	...
	public static function get_icon_manager_tabs_config() {
		$tabs = [
			'all' => [
				'name' => 'all',
				'label' => __( 'All Icons', 'madxartwork' ),
	 ...
	 * Render Icon
	 *
	 * Used to render Icon for \madxartwork\Controls_Manager::ICONS
	...
	public static function fa4_to_fa5_value_migration( $value ) {
		...
		if ( false === $migration_dictionary ) {
			$migration_dictionary = json_decode( file_get_contents( madxartwork_ASSETS_PATH . 'lib/font-awesome/migration/mapping.js' ), true );
		}
	...
	public static function is_migration_allowed() {
		...
		if ( false === $migration_allowed ) {
			 ...
			 */
			$migration_allowed = apply_filters( 'madxartwork/icons_manager/migration_allowed', $migration_allowed );
		}
	...
	public function register_admin_settings( Settings $settings ) {
		$settings->add_field(
			...
			'load_fa4_shim',
			[
				'label' => __( 'Load Font Awesome 4 Support', 'madxartwork' ),
				'field_args' => [
					'type' => 'select',
					'std' => 1,
					'options' => [
					...
					'desc' => __( 'Font Awesome 4 support script (shim.js) is a script that makes sure all previously selected Font Awesome 4 icons are displayed correctly while using Font Awesome 5 library.', 'madxartwork' ),
				],
			]
		);
	}
	...
	public function register_admin_tools_settings( Tools $settings ) {
		$settings->add_tab( 'fontawesome4_migration', [ 'label' => __( 'Font Awesome Upgrade', 'madxartwork' ) ] );

		$settings->add_section( 'fontawesome4_migration', 'fontawesome4_migration', [
			'callback' => function() {
				echo '<h2>' . esc_html__( 'Font Awesome Upgrade', 'madxartwork' ) . '</h2>';
				echo '<p>' .
				esc_html__( 'Access 1,500+ amazing Font Awesome 5 icons and enjoy faster performance and design flexibility.', 'madxartwork' ) . '<br>' .
				esc_html__( 'By upgrading, whenever you edit a page containing a Font Awesome 4 icon, madxartwork will convert it to the new Font Awesome 5 icon.', 'madxartwork' ) .
				'</p><p><strong>' .
				esc_html__( 'Please note that the upgrade process may cause some of the previously used Font Awesome 4 icons to look a bit different due to minor design changes made by Font Awesome.', 'madxartwork' ) .
				'</strong></p><p>' .
				esc_html__( 'This action is not reversible and cannot be undone by rolling back to previous versions.', 'madxartwork' ) .
				'</p>';
			},
			'fields' => [
				[
					'label'      => __( 'Font Awesome Upgrade', 'madxartwork' ),
					'field_args' => [
						'type' => 'raw_html',
						'html' => sprintf( '<span data-action="%s" data-_nonce="%s" class="button" id="madxartwork_upgrade_fa_button">%s</span>',
							self::NEEDS_UPDATE_OPTION . '_upgrade',
							wp_create_nonce( self::NEEDS_UPDATE_OPTION ),
							__( 'Upgrade To Font Awesome 5', 'madxartwork' )
						),
					],
				],
			],
	...
	public function ajax_upgrade_to_fa5() {
		...
		delete_option( 'madxartwork_' . self::NEEDS_UPDATE_OPTION );

		wp_send_json_success( [ 'message' => '<p>' . __( 'Hurray! The upgrade process to Font Awesome 5 was completed successfully.', 'madxartwork' ) . '</p>' ] );
	}
	...
	public function enqueue_fontawesome_css() {
		if ( ! self::is_migration_allowed() ) {
			...
			$load_shim = get_option( 'madxartwork_load_fa4_shim', false );
			if ( 'madxartwork/editor/after_enqueue_styles' === $current_filter ) {
				self::enqueue_shim();
	...
	public function add_admin_strings( $settings ) {
		$settings['i18n']['confirm_fa_migration_admin_modal_body']  = __( 'I understand that by upgrading to Font Awesome 5,', 'madxartwork' ) . '<br>' . __( 'I acknowledge that some changes may affect my website and that this action cannot be undone.', 'madxartwork' );
		$settings['i18n']['confirm_fa_migration_admin_modal_head']  = __( 'Font Awesome 5 Migration', 'madxartwork' );
		return $settings;
	}
	...
	public function ajax_enable_svg_uploads() {
		update_option( 'madxartwork_allow_svg', 1 );
	}
	...
	public function __construct() {
		if ( is_admin() ) {
			...
			add_action( 'madxartwork/admin/after_create_settings/' . Settings::PAGE_ID, [ $this, 'register_admin_settings' ], 100 );
			add_action( 'madxartwork/admin/localize_settings', [ $this, 'add_admin_strings' ] );
		}

		add_action( 'madxartwork/editor/after_enqueue_styles', [ $this, 'enqueue_fontawesome_css' ] );
		add_action( 'madxartwork/frontend/after_enqueue_styles', [ $this, 'enqueue_fontawesome_css' ] );

		add_action( 'madxartwork/frontend/after_register_styles', [ $this, 'register_styles' ] );

		// Ajax.
		add_action( 'madxartwork/ajax/register_actions', [ $this, 'register_ajax_actions' ] );

		if ( ! self::is_migration_allowed() ) {
			add_filter( 'madxartwork/editor/localize_settings', [ $this, 'add_update_needed_flag' ] );
			add_action( 'madxartwork/admin/after_create_settings/' . Tools::PAGE_ID, [ $this, 'register_admin_tools_settings' ], 100 );
