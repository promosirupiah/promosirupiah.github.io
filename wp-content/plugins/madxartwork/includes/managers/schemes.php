...
/**
 * madxartwork scheme manager.
 *
 * madxartwork scheme manager handler class is responsible for registering and
...
class Schemes_Manager {
	 ...
	 * Ajax apply scheme.
	 *
	 * Ajax handler for madxartwork apply_scheme.
	 *
	 * Fired by `wp_ajax_madxartwork_apply_scheme` action.
	 *
	 * @since 1.0.0
	 * @access public
	 */
	...
	public static function get_enabled_schemes() {
		if ( null === self::$_enabled_schemes ) {
			...
			foreach ( self::$_schemes_types as $schemes_type => $scheme_class ) {
				if ( 'yes' === get_option( 'madxartwork_disable_' . $schemes_type . '_schemes' ) ) {
					continue;
				}
			 ...
			 */
			$enabled_schemes = apply_filters( 'madxartwork/schemes/enabled_schemes', $enabled_schemes );
	 ...
	 *
	 * Initializing madxartwork schemes manager and register default schemes.
	 *
	 * @since 1.0.0
	 * @access public
	 */
