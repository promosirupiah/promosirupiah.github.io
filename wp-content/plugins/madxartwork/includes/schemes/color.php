...
/**
 * madxartwork color scheme.
 *
 * madxartwork color scheme class is responsible for initializing a scheme for
 * colors.
 *
 * @since 1.0.0
 */
class Scheme_Color extends Scheme_Base {
	...
	public function get_disabled_title() {
		return __( 'Color Palettes', 'madxartwork' );
	}
	...
	public function get_scheme_titles() {
		return [
			self::COLOR_1 => __( 'Primary', 'madxartwork' ),
			self::COLOR_2 => __( 'Secondary', 'madxartwork' ),
			self::COLOR_3 => __( 'Text', 'madxartwork' ),
			self::COLOR_4 => __( 'Accent', 'madxartwork' ),
		];
	}
	...
	public function print_template_content() {
		?>
		<div class="madxartwork-panel-scheme-content madxartwork-panel-box">
			<div class="madxartwork-panel-heading">
				<div class="madxartwork-panel-heading-title"><?php echo $this->_get_current_scheme_title(); ?></div>
			</div>
			<?php
			...
			if ( $description ) :
				?>
				<div class="madxartwork-panel-scheme-description madxartwork-descriptor"><?php echo $description; ?></div>
			<?php endif; ?>
			<div class="madxartwork-panel-scheme-items madxartwork-panel-box-content"></div>
		</div>
		<div class="madxartwork-panel-scheme-colors-more-palettes madxartwork-panel-box">
			<div class="madxartwork-panel-heading">
				<div class="madxartwork-panel-heading-title"><?php echo __( 'More Palettes', 'madxartwork' ); ?></div>
			</div>
			<div class="madxartwork-panel-box-content">
				<?php foreach ( $this->_get_system_schemes_to_print() as $scheme_name => $scheme ) : ?>
					<div class="madxartwork-panel-scheme-color-system-scheme" data-scheme-name="<?php echo esc_attr( $scheme_name ); ?>">
						<div class="madxartwork-panel-scheme-color-system-items">
							<?php foreach ( $scheme['items'] as $color_value ) : ?>
								<div class="madxartwork-panel-scheme-color-system-item" style="background-color: <?php echo esc_attr( $color_value ); ?>;"></div>
							<?php endforeach; ?>
						</div>
						<div class="madxartwork-title"><?php echo $scheme['title']; ?></div>
					</div>
				<?php endforeach; ?>
			</div>
		</div>
	...
	protected function _get_current_scheme_title() {
		return __( 'Color Palette', 'madxartwork' );
	}
}
