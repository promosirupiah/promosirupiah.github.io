...
abstract class Scheme_Base implements Scheme_Interface {
	 ...
	 */
	const LAST_UPDATED_META = '_madxartwork_scheme_last_updated';
	...
	public function get_scheme_value() {
		$scheme_value = get_option( 'madxartwork_scheme_' . static::get_type() );

		if ( ! $scheme_value ) {
			$scheme_value = $this->get_default_scheme();

			update_option( 'madxartwork_scheme_' . static::get_type(), $scheme_value );
		}
	 ...
	 * Save scheme.
	 *
	 * Update madxartwork scheme in the database, and update the last updated
	 * scheme time.
	 *
	 * @since 1.0.0
	 * @access public
	...
	public function save_scheme( array $posted ) {
		...
		update_option( 'madxartwork_scheme_' . static::get_type(), array_replace( $scheme_value, array_intersect_key( $posted, $scheme_value ) ) );
	...
	final public function print_template() {
		?>
		<script type="text/template" id="tmpl-madxartwork-panel-schemes-<?php echo static::get_type(); ?>">
			<div class="madxartwork-panel-scheme-buttons">
				<div class="madxartwork-panel-scheme-button-wrapper madxartwork-panel-scheme-reset">
					<button class="madxartwork-button">
				...
				<div class="madxartwork-panel-scheme-button-wrapper madxartwork-panel-scheme-save">
					<button class="madxartwork-button madxartwork-button-success" disabled><?php echo __( 'Apply', 'madxartwork' ); ?></button>
				</div>
			</div>
