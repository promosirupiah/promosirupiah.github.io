...
/**
 * madxartwork database.
 *
 * madxartwork database handler class is responsible for communicating with the
 * DB, save and retrieve madxartwork data and meta data.
 *
 * @since 1.0.0
 */
class DB {
	 ...
	 *
	 * When editing the with madxartwork the first time, the current page content
	 * is parsed into Text Editor Widget that contains the original data.
	 *
	 * @since 2.1.0
	 * @deprecated 2.3.0 Use `Plugin::$instance->documents->get( $post_id )->convert_to_madxartwork()` instead
	 * @access public
	 *
	 * @param int $post_id Post ID.
	 *
	 * @return array Content in madxartwork format.
	 */
	public function get_new_editor_from_wp_editor( $post_id ) {
		 _deprecated_function( __METHOD__, '2.3.0', 'Plugin::$instance->documents->get( $post_id )->convert_to_madxartwork()' );

		$document = Plugin::$instance->documents->get( $post_id );

		if ( $document ) {
			return $document->convert_to_madxartwork();
		}
	 ...
	 *
	 * @since 1.0.0
	 * @deprecated 2.1.0 Use `Plugin::$instance->documents->get( $post_id )->convert_to_madxartwork()` instead
	 * @access public
	 *
	 * @param int $post_id Post ID.
	 *
	...
	public function _get_new_editor_from_wp_editor( $post_id ) {
		_deprecated_function( __METHOD__, '2.1.0', 'Plugin::$instance->documents->get( $post_id )->convert_to_madxartwork()' );

		return $this->get_new_editor_from_wp_editor( $post_id );
	}

	/**
	 * Is using madxartwork.
	 *
	 * Set whether the page is using madxartwork or not.
	 *
	 * @since 1.5.0
	 * @access public
	 *
	 * @param int  $post_id      Post ID.
	 * @param bool $is_madxartwork Optional. Whether the page is madxartwork page.
	 *                           Default is true.
	 */
	public function set_is_madxartwork_page( $post_id, $is_madxartwork = true ) {
		if ( $is_madxartwork ) {
			// Use the string `builder` and not a boolean for rollback compatibility
			update_post_meta( $post_id, '_madxartwork_edit_mode', 'builder' );
		} else {
			delete_post_meta( $post_id, '_madxartwork_edit_mode' );
		}
	}
	 ...
	 * Iterate data.
	 *
	 * Accept any type of madxartwork data and a callback function. The callback
	 * function runs recursively for each element and his child elements.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 * @param array    $data_container Any type of madxartwork data.
	...
	/**
	 * Safely copy madxartwork meta.
	 *
	 * Make sure the original page was built with madxartwork and the post is not
	 * auto-save. Only then copy madxartwork meta from one post to another using
	 * `copy_madxartwork_meta()`.
	 *
	 * @since 1.9.2
	 * @access public
	 *
	 ...
	 */
	public function safe_copy_madxartwork_meta( $from_post_id, $to_post_id ) {
		// It's from  WP-Admin & not from madxartwork.
		if ( ! did_action( 'madxartwork/db/before_save' ) ) {

			if ( ! Plugin::$instance->db->is_built_with_madxartwork( $from_post_id ) ) {
				return;
			}

			// It's an exited madxartwork auto-save
			if ( get_post_meta( $to_post_id, '_madxartwork_data', true ) ) {
				return;
			}
		}

		$this->copy_madxartwork_meta( $from_post_id, $to_post_id );
	}

	/**
	 * Copy madxartwork meta.
	 *
	 * Duplicate the data from one post to another.
	 *
	 * Consider using `safe_copy_madxartwork_meta()` method instead.
	 *
	 * @since 1.1.0
	 * @access public
	 *
	 ...
	 * @param int $to_post_id   Target post ID.
	 */
	public function copy_madxartwork_meta( $from_post_id, $to_post_id ) {
		...
		foreach ( $from_post_meta as $meta_key => $values ) {
			// Copy only meta with the `_madxartwork` prefix
			if ( 0 === strpos( $meta_key, '_madxartwork' ) || in_array( $meta_key, $core_meta, true ) ) {
				$value = $values[0];

				// The madxartwork JSON needs slashes before saving
				if ( '_madxartwork_data' === $meta_key ) {
	 ...
	 */
	public function is_built_with_madxartwork( $post_id ) {
		return ! ! get_post_meta( $post_id, '_madxartwork_edit_mode', true );
	}
	 ...
	 *
	 * Retrieve the post plain text from any given madxartwork data.
	 *
	 * @since 1.9.2
	 * @access public
	 *
