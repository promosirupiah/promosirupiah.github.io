...
foreach ( $reports as $report_name => $report ) : ?>
	<div class="madxartwork-system-info-section madxartwork-system-info-<?php echo esc_attr( $report_name ); ?>">
		<table class="widefat">
			<thead>
			<tr>
			...
			foreach ( $report['report'] as $field_name => $field ) :
				if ( in_array( $report_name, [ 'plugins', 'network_plugins', 'mu_plugins' ], true ) ) {
					...
					$warning_class = ! empty( $field['warning'] ) ? ' class="madxartwork-warning"' : '';
