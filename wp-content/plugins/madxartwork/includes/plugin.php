...
use madxartwork\Core\Admin\Admin;
use madxartwork\Core\Common\Modules\Ajax\Module as Ajax;
use madxartwork\Core\Common\App as CommonApp;
use madxartwork\Core\Debug\Inspector;
use madxartwork\Core\Documents_Manager;
use madxartwork\Core\Editor\Editor;
use madxartwork\Core\Files\Manager as Files_Manager;
use madxartwork\Core\Files\Assets\Manager as Assets_Manager;
use madxartwork\Core\Modules_Manager;
use madxartwork\Core\Settings\Manager as Settings_Manager;
use madxartwork\Core\Settings\Page\Manager as Page_Settings_Manager;
use madxartwork\Modules\History\Revisions_Manager;
use madxartwork\Core\DynamicTags\Manager as Dynamic_Tags_Manager;
use madxartwork\Core\Logger\Manager as Log_Manager;
 ...
 * madxartwork plugin.
 *
 * The main plugin handler class is responsible for initializing madxartwork. The
...
class Plugin {
	 ...
	 * @access public
	 *
	 * @var \madxartwork\Core\RoleManager\Role_Manager
	 */
	public $role_manager;
	...
	public function __clone() {
		...
		_doing_it_wrong( __FUNCTION__, esc_html__( 'Something went wrong.', 'madxartwork' ), '1.0.0' );
	}
	...
	public static function instance() {
		if ( is_null( self::$instance ) ) {
			 ...
			 * madxartwork loaded.
			 *
			 * Fires when madxartwork was fully loaded and instantiated.
			 *
			 * @since 1.0.0
			 */
			do_action( 'madxartwork/loaded' );
		}
	 ...
	 * Init.
	 *
	 * Initialize madxartwork Plugin. Register madxartwork support for all the
	 * supported post types and initialize madxartwork components.
	 *
	 * @since 1.0.0
	 * @access public
	 */
	public function init() {
		 ...
		 * madxartwork init.
		 *
		 * Fires on madxartwork init, after madxartwork has finished loading but
		 * before any headers are sent.
		 *
		 * @since 1.0.0
		 */
		do_action( 'madxartwork/init' );
	}

	/**
	 * Get install time.
	 *
	 * Retrieve the time when madxartwork was installed.
	 *
	 * @since 2.6.0
	 * @access public
	 * @static
	 *
	 * @return int Unix timestamp when madxartwork was installed.
	 */
	public function get_install_time() {
		$installed_time = get_option( '_madxartwork_installed_time' );

		if ( ! $installed_time ) {
			$installed_time = time();

			update_option( '_madxartwork_installed_time', $installed_time );
		}
	 ...
	 * Init components.
	 *
	 * Initialize madxartwork components. Register actions, run setting manager,
	 * initialize all the components that run madxartwork, and if in admin page
	 ...
	 *
	 * Register madxartwork support for all the supported post types defined by
	 * the user in the admin screen and saved as `madxartwork_cpt_support` option
	...
	private function add_cpt_support() {
		$cpt_support = get_option( 'madxartwork_cpt_support', [ 'page', 'post' ] );

		foreach ( $cpt_support as $cpt_slug ) {
			add_post_type_support( $cpt_slug, 'madxartwork' );
		}
	}
	 ...
	 * Register autoloader.
	 *
	 * madxartwork autoloader loads all the classes needed to run the plugin.
	 *
	 * @since 1.6.0
	 * @access private
	 */
	private function register_autoloader() {
		require madxartwork_PATH . '/includes/autoloader.php';
	 ...
	 * Plugin constructor.
	 *
	 * Initializing madxartwork plugin.
	 *
	 * @since 1.0.0
	 * @access private
	 */
