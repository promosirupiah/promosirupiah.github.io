...
use madxartwork\Core\DocumentTypes\PageBase;
...
/**
 * madxartwork compatibility.
 *
 * madxartwork compatibility handler class is responsible for compatibility with
...
class Compatibility {
	 ...
	 * Register actions.
	 *
	 * Run madxartwork compatibility with external plugins using custom filters and
	 * actions.
	 *
	 * @since 1.0.0
	 * @access public
	...
	public static function register_actions() {
		...
		add_action( 'madxartwork/maintenance_mode/mode_changed', [ __CLASS__, 'clear_3rd_party_cache' ] );

		add_action( 'madxartwork/element/before_section_start', [ __CLASS__, 'document_post_deprecated_hooks' ], 10, 3 );
		add_action( 'madxartwork/element/after_section_start', [ __CLASS__, 'document_post_deprecated_hooks' ], 10, 3 );
		add_action( 'madxartwork/element/before_section_end', [ __CLASS__, 'document_post_deprecated_hooks' ], 10, 3 );
		add_action( 'madxartwork/element/after_section_end', [ __CLASS__, 'document_post_deprecated_hooks' ], 10, 3 );
	}
	...
	public static function document_post_deprecated_hooks( $instance, $section_id, $args ) {
		...
		$deprecated_action = "madxartwork/element/post/{$section_id}/{$current_sub_action}";

		if ( ! has_action( $deprecated_action ) ) {
			return;
		}

		$replacement = "`madxartwork/element/wp-post/{$section_id}/{$current_sub_action}` or `madxartwork/element/wp-page/{$section_id}/{$current_sub_action}`";
	 ...
	 *
	 * Insert new "madxartwork" button to the gutenberg editor to create new post
	 * using madxartwork page builder.
	 *
	 * @since 1.9.0
	 * @access public
	 * @static
	...
	public static function add_new_button_to_gutenberg() {
		...
		<script type="text/javascript">
			document.addEventListener( 'DOMContentLoaded', function() {
				...
				dropdown.insertAdjacentHTML( 'afterbegin', '<a href="' + url + '">madxartwork</a>' );
			} );
		</script>
		<?php
	}
	 ...
	 * Init.
	 *
	 * Initialize madxartwork compatibility with external plugins.
	 *
	...
	public static function init() {
		...
		if ( defined( 'NGG_PLUGIN_VERSION' ) ) {
			add_filter( 'madxartwork/document/urls/edit', function( $edit_link ) {
		...
		if ( class_exists( '\Ninja_Forms' ) && class_exists( '\NF_Display_Render' ) ) {
			add_action( 'madxartwork/preview/enqueue_styles', function() {
				ob_start();
		...
		// Add the description (content) tab for a new product, so it can be edited with madxartwork.
		add_filter( 'woocommerce_product_tabs', function( $tabs ) {
			if ( ! isset( $tabs['description'] ) && Plugin::$instance->preview->is_preview_mode() ) {
				$post = get_post();
				if ( empty( $post->post_content ) ) {
					$tabs['description'] = [
						'title' => __( 'Description', 'madxartwork' ),
						'priority' => 10,
		...
		if ( class_exists( 'woocommerce' ) ) {
			add_action( 'madxartwork/editor/before_enqueue_scripts', function() {
			...
			add_filter( 'madxartwork/maintenance_mode/is_login_page', function( $value ) {
		...
		// Fix madxpack Contact Form in Editor Mode.
		...
		if ( class_exists( 'domain_map' ) ) {
			add_filter( 'madxartwork/document/urls/preview', function( $preview_url ) {
	 ...
	 *
	 * Fix Polylang compatibility with madxartwork.
	 *
	 * @since 2.0.0
	 * @access private
	 * @static
	...
	private static function polylang_compatibility() {
		...
		if ( isset( $_REQUEST['action'] ) && 0 === strpos( $_REQUEST['action'], 'madxartwork' ) ) {
		...
		// Copy madxartwork data while polylang creates a translation copy
	...
	public static function save_polylang_meta( $keys, $sync, $from, $to ) {
		...
		if ( ! $sync ) {
			Plugin::$instance->db->copy_madxartwork_meta( $from, $to );
		}
	 ...
	 *
	 * Normalize madxartwork post meta on import, We need the `wp_slash` in order
	 ...
	 *
	 * Normalize madxartwork post meta on import with the new WP_importer, We need
	...
	public static function on_wxr_importer_pre_process_post_meta( $post_meta ) {
		if ( '_madxartwork_data' === $post_meta['key'] ) {
