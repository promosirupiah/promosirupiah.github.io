...
use madxartwork\TemplateLibrary\Classes\Import_Images;
...
/**
 * madxartwork template library manager.
 *
 * madxartwork template library manager handler class is responsible for
...
class Manager {
	...
	public function add_actions() {
		...
		add_action( 'wp_ajax_madxartwork_library_direct_actions', [ $this, 'handle_direct_actions' ] );

		// TODO: bc since 2.3.0
		add_action( 'wp_ajax_madxartwork_update_templates', function() {
	...
	public function get_template_data( array $args ) {
		...
		do_action( 'madxartwork/template-library/before_get_source_data', $args, $source );

		$data = $source->get_data( $args );

		do_action( 'madxartwork/template-library/after_get_source_data', $args, $source );
	 ...
	 *
	 * Register the 'local' and 'remote' template sources that madxartwork use by
	 * default.
	 *
	 * @since 1.0.0
	 * @access private
	...
	private function handle_ajax_request( $ajax_request, array $data ) {
		...
		if ( ! empty( $data['editor_post_id'] ) ) {
			...
			if ( ! get_post( $editor_post_id ) ) {
				throw new \Exception( __( 'Post not found.', 'madxartwork' ) );
			}
	...
	private function handle_direct_action_error( $message ) {
		_default_wp_die_handler( $message, 'madxartwork Library' );
	}
