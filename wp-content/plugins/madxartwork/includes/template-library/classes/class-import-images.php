<?php
namespace madxartwork\TemplateLibrary\Classes;
...
/**
 * madxartwork template library import images.
 *
 * madxartwork template library import images handler class is responsible for
...
class Import_Images {
		...
		$post_id = $wpdb->get_var(
			$wpdb->prepare(
				'SELECT `post_id` FROM `' . $wpdb->postmeta . '`
					WHERE `meta_key` = \'_madxartwork_source_image_hash\'
						AND `meta_value` = %s
				;',
	...
	public function import( $attachment ) {
		...
		if ( $info ) {
			...
			return $attachment;
			// return new \WP_Error( 'attachment_processing_error', __( 'Invalid file type.', 'madxartwork' ) );
		}
		...
		update_post_meta( $post_id, '_madxartwork_source_image_hash', $this->get_hash_image( $attachment['url'] ) );
