...
/**
 * madxartwork testimonial widget.
 *
 * madxartwork widget that displays customer testimonials that show social proof.
 *
 * @since 1.0.0
 */
class Widget_Testimonial extends Widget_Base {
	...
	public function get_title() {
		return __( 'Testimonial', 'madxartwork' );
	}
	...
	protected function _register_controls() {
		$this->start_controls_section(
			'section_testimonial',
			[
				'label' => __( 'Testimonial', 'madxartwork' ),
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .madxartwork-testimonial-content' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .madxartwork-testimonial-content',
			]
		);
		...
		$this->add_control(
			'image_size',
			[
				'label' => __( 'Image Size', 'madxartwork' ),
				...
				'selectors' => [
					'{{WRAPPER}} .madxartwork-testimonial-wrapper .madxartwork-testimonial-image img' => 'width: {{SIZE}}{{UNIT}};height: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				'name' => 'image_border',
				'selector' => '{{WRAPPER}} .madxartwork-testimonial-wrapper .madxartwork-testimonial-image img',
				'separator' => 'before',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px', '%' ],
				'selectors' => [
					'{{WRAPPER}} .madxartwork-testimonial-wrapper .madxartwork-testimonial-image img' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .madxartwork-testimonial-name' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .madxartwork-testimonial-name',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .madxartwork-testimonial-job' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .madxartwork-testimonial-job',
			]
		);
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', 'madxartwork-testimonial-wrapper' );

		if ( $settings['testimonial_alignment'] ) {
			$this->add_render_attribute( 'wrapper', 'class', 'madxartwork-testimonial-text-align-' . $settings['testimonial_alignment'] );
		}

		$this->add_render_attribute( 'meta', 'class', 'madxartwork-testimonial-meta' );

		if ( $settings['testimonial_image']['url'] ) {
			$this->add_render_attribute( 'meta', 'class', 'madxartwork-has-image' );
		}

		if ( $settings['testimonial_image_position'] ) {
			$this->add_render_attribute( 'meta', 'class', 'madxartwork-testimonial-image-position-' . $settings['testimonial_image_position'] );
		}
		...
		<div <?php echo $this->get_render_attribute_string( 'wrapper' ); ?>>
			<?php
			if ( $has_content ) :
				$this->add_render_attribute( 'testimonial_content', 'class', 'madxartwork-testimonial-content' );
			...
			<div <?php echo $this->get_render_attribute_string( 'meta' ); ?>>
				<div class="madxartwork-testimonial-meta-inner">
					<?php if ( $has_image ) : ?>
						<div class="madxartwork-testimonial-image">
							<?php
					...
					<div class="madxartwork-testimonial-details">
						<?php
						if ( $has_name ) :
							$this->add_render_attribute( 'testimonial_name', 'class', 'madxartwork-testimonial-name' );
						...
						<?php
						if ( $has_job ) :
							$this->add_render_attribute( 'testimonial_job', 'class', 'madxartwork-testimonial-job' );
	...
	protected function _content_template() {
		...
		if ( '' !== settings.testimonial_image.url ) {
			imageUrl = madxartwork.imagesManager.getImageUrl( image );
			hasImage = ' madxartwork-has-image';
		...
		var testimonial_alignment = settings.testimonial_alignment ? ' madxartwork-testimonial-text-align-' + settings.testimonial_alignment : '';
		var testimonial_image_position = settings.testimonial_image_position ? ' madxartwork-testimonial-image-position-' + settings.testimonial_image_position : '';
		#>
		<div class="madxartwork-testimonial-wrapper{{ testimonial_alignment }}">
			<# if ( '' !== settings.testimonial_content ) {
				view.addRenderAttribute( 'testimonial_content', 'class', 'madxartwork-testimonial-content' );
			...
			<# } #>
			<div class="madxartwork-testimonial-meta{{ hasImage }}{{ testimonial_image_position }}">
				<div class="madxartwork-testimonial-meta-inner">
					<# if ( imageUrl ) { #>
					<div class="madxartwork-testimonial-image">{{{ imageHtml }}}</div>
					<# } #>

					<div class="madxartwork-testimonial-details">
						<# if ( '' !== settings.testimonial_name ) {
							view.addRenderAttribute( 'testimonial_name', 'class', 'madxartwork-testimonial-name' );
						...
						<# if ( '' !== settings.testimonial_job ) {
							view.addRenderAttribute( 'testimonial_job', 'class', 'madxartwork-testimonial-job' );
