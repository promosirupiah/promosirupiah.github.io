...
/**
 * madxartwork image size control.
 *
...
class Group_Control_Image_Size extends Group_Control_Base {
	...
	public static function get_attachment_image_html( $settings, $image_size_key = 'image', $image_key = null ) {
		...
		$image_class = ! empty( $settings['hover_animation'] ) ? 'madxartwork-animation-' . $settings['hover_animation'] : '';
		 ...
		 *                               as the image key.
		 */
		return apply_filters( 'madxartwork/image_size/get_attachment_image_html', $html, $settings, $image_size_key, $image_key );
	}
	...
	protected function init_fields() {
		...
		$fields['size'] = [
			'label' => _x( 'Image Size', 'Image Size Control', 'madxartwork' ),
			'type' => Controls_Manager::SELECT,
			'label_block' => false,
		];

		$fields['custom_dimension'] = [
			'label' => _x( 'Image Dimension', 'Image Size Control', 'madxartwork' ),
			'type' => Controls_Manager::IMAGE_DIMENSIONS,
			'description' => __( 'You can crop the original image size to any custom size. You can also set a single value for height or width in order to keep the original size ratio.', 'madxartwork' ),
			'condition' => [
				'size' => 'custom',
			],
			'separator' => 'none',
	...
	private function get_image_sizes() {
		...
		$image_sizes['full'] = _x( 'Full', 'Image Size Control', 'madxartwork' );

		if ( ! empty( $args['include']['custom'] ) || ! in_array( 'custom', $args['exclude'] ) ) {
			$image_sizes['custom'] = _x( 'Custom', 'Image Size Control', 'madxartwork' );
		}
