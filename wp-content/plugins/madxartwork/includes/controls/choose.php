	...
	public function content_template() {
		...
		<div class="madxartwork-control-field">
			...
			<div class="madxartwork-control-input-wrapper">
				<div class="madxartwork-choices">
					<# _.each( data.options, function( options, value ) { #>
					<input id="<?php echo $control_uid; ?>" type="radio" name="madxartwork-choose-{{ data.name }}-{{ data._cid }}" value="{{ value }}">
					<label class="madxartwork-choices-label tooltip-target" for="<?php echo $control_uid; ?>" data-tooltip="{{ options.title }}" title="{{ options.title }}">
						<i class="{{ options.icon }}" aria-hidden="true"></i>
						<span class="madxartwork-screen-only">{{{ options.title }}}</span>
					</label>
					<# } ); #>
				</div>
			</div>
