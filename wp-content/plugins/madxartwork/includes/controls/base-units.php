...
/**
 * madxartwork control base units.
 *
...
abstract class Control_Base_Units extends Control_Base_Multiple {
	...
	protected function print_units_template() {
		...
		<div class="madxartwork-units-choices">
			<# _.each( data.size_units, function( unit ) { #>
			<input id="madxartwork-choose-{{ data._cid + data.name + unit }}" type="radio" name="madxartwork-choose-{{ data.name }}" data-setting="unit" value="{{ unit }}">
			<label class="madxartwork-units-choices-label" for="madxartwork-choose-{{ data._cid + data.name + unit }}">{{{ unit }}}</label>
			<# } ); #>
		</div>
		<# } #>
		<?php
