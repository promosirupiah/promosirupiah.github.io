...
/**
 * madxartwork history revisions manager.
 *
 * madxartwork history revisions manager handler class is responsible for
 * registering and managing madxartwork revisions manager.
 *
 * @since 1.7.0
 */
class Revisions_Manager {
	 ...
	 *
	 * Initializing madxartwork history revisions manager.
	 *
	 * @since 1.7.0
	 * @access public
	 */
	...
	public static function avoid_delete_auto_save( $post_content, $post_id ) {
		...
		if ( $post_id && Plugin::$instance->db->is_built_with_madxartwork( $post_id ) ) {
	...
	public static function remove_temp_post_content() {
		...
		if ( Plugin::$instance->db->is_built_with_madxartwork( $post->ID ) ) {
			$post->post_content = str_replace( '<!-- Created with madxartwork -->', '', $post->post_content );
		}
	}
	...
	public static function get_revisions( $post_id = 0, $query_args = [], $parse_result = true ) {
		...
		foreach ( $posts as $revision ) {
			$date = date_i18n( _x( 'M j @ H:i', 'revision date format', 'madxartwork' ), strtotime( $revision->post_modified ) );
	...
	public static function save_revision( $revision_id ) {
		...
		if ( $parent_id ) {
			Plugin::$instance->db->safe_copy_madxartwork_meta( $parent_id, $revision_id );
		}
	}
	...
	public static function restore_revision( $parent_id, $revision_id ) {
		$is_built_with_madxartwork = Plugin::$instance->db->is_built_with_madxartwork( $revision_id );

		Plugin::$instance->db->set_is_madxartwork_page( $parent_id, $is_built_with_madxartwork );

		if ( ! $is_built_with_madxartwork ) {
			return;
		}

		Plugin::$instance->db->copy_madxartwork_meta( $revision_id, $parent_id );
	...
	public static function ajax_get_revision_data( array $data ) {
		...
		if ( ! current_user_can( 'edit_post', $revision->get_id() ) ) {
			throw new \Exception( __( 'Access denied.', 'madxartwork' ) );
		}
	...
	public static function ajax_delete_revision( array $data ) {
		...
		if ( ! $deleted || is_wp_error( $deleted ) ) {
			throw new \Exception( __( 'Cannot delete this revision.', 'madxartwork' ) );
		}
	}
	...
	public static function editor_settings( $settings, $post_id ) {
		$settings = array_replace_recursive( $settings, [
			...
			'current_revision_id' => self::current_revision_id( $post_id ),
			'i18n' => [
				'edit_draft' => __( 'Edit Draft', 'madxartwork' ),
				'edit_published' => __( 'Edit Published', 'madxartwork' ),
				'no_revisions_1' => __( 'Revision history lets you save your previous versions of your work, and restore them any time.', 'madxartwork' ),
				'no_revisions_2' => __( 'Start designing your page and you\'ll be able to see the entire revision history here.', 'madxartwork' ),
				'current' => __( 'Current Version', 'madxartwork' ),
				'restore' => __( 'Restore', 'madxartwork' ),
				'restore_auto_saved_data' => __( 'Restore Auto Saved Data', 'madxartwork' ),
				'restore_auto_saved_data_message' => __( 'There is an autosave of this post that is more recent than the version below. You can restore the saved data fron the Revisions panel', 'madxartwork' ),
				'revision' => __( 'Revision', 'madxartwork' ),
				'revision_history' => __( 'Revision History', 'madxartwork' ),
				'revisions_disabled_1' => __( 'It looks like the post revision feature is unavailable in your website.', 'madxartwork' ),
				'revisions_disabled_2' => sprintf(
					/* translators: %s: Codex URL */
					__( 'Learn more about <a target="_blank" href="%s">WordPress revisions</a>', 'madxartwork' ),
	...
	private static function register_actions() {
		...
		add_filter( 'madxartwork/editor/localize_settings', [ __CLASS__, 'editor_settings' ], 10, 2 );
		add_action( 'madxartwork/db/before_save', [ __CLASS__, 'db_before_save' ], 10, 2 );
		...
		if ( wp_doing_ajax() ) {
			add_filter( 'madxartwork/documents/ajax_save/return_data', [ __CLASS__, 'on_ajax_save_builder_data' ], 10, 2 );
		}
	}
