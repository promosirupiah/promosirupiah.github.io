...
abstract class DB_Upgrades_Manager extends Background_Task_Manager {
	...
	public function get_task_runner_class() {
		return 'madxartwork\Core\Upgrade\Updater';
	}
	...
	public function on_runner_start() {
		...
		define( 'IS_madxartwork_UPGRADE', true );
	}

	public function on_runner_complete( $did_tasks = false ) {
		$logger = Plugin::$instance->logger->get_logger();

		$logger->info( 'madxartwork data updater process has been completed.', [
			'meta' => [
	...
	public function admin_notice_start_upgrade() {
		...
		$message = '<p>' . sprintf( __( '%s Your site database needs to be updated to the latest version.', 'madxartwork' ), $this->get_updater_label() ) . '</p>';
		$message .= '<p>' . sprintf( '<a href="%s" class="button-primary">%s</a>', $upgrade_link, __( 'Update Now', 'madxartwork' ) ) . '</p>';
	...
	public function admin_notice_upgrade_is_running() {
		...
		$message = '<p>' . sprintf( __( '%s Database update process is running in the background.', 'madxartwork' ), $this->get_updater_label() ) . '</p>';
	...
	public function admin_notice_upgrade_is_completed() {
		...
		$message = '<p>' . sprintf( __( '%s The database update process is now complete. Thank you for updating to the latest version!', 'madxartwork' ), $this->get_updater_label() ) . '</p>';
	...
	protected function start_run() {
		...
		Plugin::$instance->logger->get_logger()->info( 'madxartwork data updater process has been queued.', [
			'meta' => [
				'plugin' => $this->get_plugin_label(),
				'from' => $this->current_version,
				'to' => $this->get_new_version(),
