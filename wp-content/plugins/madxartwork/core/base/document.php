<?php
namespace madxartwork\Core\Base;

use madxartwork\Core\Files\CSS\Post as Post_CSS;
use madxartwork\Core\Utils\Exceptions;
...
abstract class Document extends Controls_Stack {
	...
	const PAGE_META_KEY = '_madxartwork_page_settings';
	...
	public static function get_editor_panel_config() {
		return [
			...
			'messages' => [
				...
				'publish_notification' => sprintf( __( 'Hurray! Your %s is live.', 'madxartwork' ), static::get_title() ),
			],
		];
	}
	...
	public static function get_title() {
		return __( 'Document', 'madxartwork' );
	}
	...
	public function get_container_attributes() {
		...
		$attributes = [
			'data-madxartwork-type' => $this->get_name(),
			'data-madxartwork-id' => $id,
			'class' => 'madxartwork madxartwork-' . $id,
		];

		$version_meta = $this->get_main_meta( '_madxartwork_version' );

		if ( version_compare( $version_meta, '2.5.0', '<' ) ) {
			$attributes['class'] .= ' madxartwork-bc-flex-widget';
		}

		if ( ! Plugin::$instance->preview->is_preview_mode( $id ) ) {
			$attributes['data-madxartwork-settings'] = wp_json_encode( $this->get_frontend_settings() );
		}
	...
	public function get_wp_preview_url() {
		 ...
		 */
		$url = apply_filters( 'madxartwork/document/urls/wp_preview', $url, $this );
	...
	public function get_exit_to_dashboard_url() {
		 ...
		 * @param Document $this The document instance.
		 */
		$url = apply_filters( 'madxartwork/document/urls/exit_to_dashboard', $url, $this );
	...
	public function get_autosave( $user_id = 0, $create = false ) {
		...
		if ( $autosave_id ) {
			...
			$autosave_id = wp_create_post_autosave( [
				...
				'post_content' => '<!-- Created With madxartwork -->',
				'post_modified' => current_time( 'mysql' ),
			] );

			Plugin::$instance->db->copy_madxartwork_meta( $this->post->ID, $autosave_id );
	...
	public function filter_admin_row_actions( $actions ) {
		if ( $this->is_built_with_madxartwork() && $this->is_editable_by_current_user() ) {
			$actions['edit_with_madxartwork'] = sprintf(
				'<a href="%1$s">%2$s</a>',
				$this->get_edit_url(),
				__( 'Edit with madxartwork', 'madxartwork' )
			);
		}
	...
	protected function _get_initial_config() {
		return [
			'id' => $this->get_main_id(),
			'type' => $this->get_name(),
			'version' => $this->get_main_meta( '_madxartwork_version' ),
	...
	protected function _register_controls() {
		$this->start_controls_section(
			'document_settings',
			[
				'label' => __( 'General Settings', 'madxartwork' ),
		...
		if ( $is_published || $can_publish || ! Plugin::$instance->editor->is_edit_mode() ) {
			...
			if ( 'future' === $this->get_main_post()->post_status ) {
				$statuses['future'] = __( 'Future', 'madxartwork' );
			}

			$this->add_control(
				'post_status',
				[
					'label' => __( 'Status', 'madxartwork' ),
		 ...
		 *
		 * Fires after madxartwork registers the document controls.
		 *
		 * @since 2.0.0
		 *
		 * @param Document $this The document instance.
		 */
		do_action( 'madxartwork/documents/register_controls', $this );
	}
	...
	public function save( $data ) {
		 ...
		 *
		 * Fires when document save starts on madxartwork.
		 *
		 * @since 2.5.12
		 *
		 * @param \madxartwork\Core\Base\Document $this The current document.
		 * @param $data.
		 */
		do_action( 'madxartwork/document/before_save', $this, $data );
		 ...
		 * @param $data.
		 */
		do_action( 'madxartwork/document/after_save', $this, $data );

		return true;
	}

	/**
	 * Is built with madxartwork.
	 *
	 * Check whether the post was built with madxartwork.
	 *
	 * @since 2.0.0
	 * @access public
	 *
	 * @return bool Whether the post was built with madxartwork.
	 */
	public function is_built_with_madxartwork() {
		return ! ! get_post_meta( $this->post->ID, '_madxartwork_edit_mode', true );
	}
	...
	public function get_edit_url() {
		 ...
		 * @param Document $this The document instance.
		 */
		$url = apply_filters( 'madxartwork/document/urls/edit', $url, $this );
	...
	public function get_preview_url() {
		...
		if ( empty( $url ) ) {
			...
			$url = set_url_scheme( add_query_arg( [
				'madxartwork-preview' => $this->get_main_id(),
				'ver' => time(),
			], $this->get_permalink() ) );
			 ...
			 * @param Document $this The document instance.
			 */
			$url = apply_filters( 'madxartwork/document/urls/preview', $url, $this );
		}
	...
	public function get_elements_data( $status = DB::STATUS_PUBLISH ) {
		$elements = $this->get_json_meta( '_madxartwork_data' );
		...
		if ( DB::STATUS_DRAFT === $status ) {
			...
			if ( is_object( $autosave ) ) {
				$autosave_elements = Plugin::$instance->documents
					...
					->get_json_meta( '_madxartwork_data' );
			}
		}

		if ( Plugin::$instance->editor->is_edit_mode() ) {
			if ( empty( $elements ) && empty( $autosave_elements ) ) {
				// Convert to madxartwork.
				$elements = $this->convert_to_madxartwork();
				if ( $this->is_autosave() ) {
					Plugin::$instance->db->copy_madxartwork_meta( $this->post->post_parent, $this->post->ID );
				}
			}
		}
	 ...
	 * @access public
	 */
	public function convert_to_madxartwork() {
		$this->save( [] );
	...
	public function get_panel_page_settings() {
		return [
			...
			'title' => sprintf( __( '%s Settings', 'madxartwork' ), static::get_title() ),
		];
	}
	...
	protected function save_elements( $elements ) {
		...
		$is_meta_updated = update_metadata( 'post', $this->post->ID, '_madxartwork_data', $json_value );

		/**
		 * Before saving data.
		 *
		 * Fires before madxartwork saves data to the database.
		 *
		 * @since 1.0.0
		 *
		 ...
		 */
		do_action( 'madxartwork/db/before_save', $this->post->post_status, $is_meta_updated );
		 ...
		 * After saving data.
		 *
		 * Fires after madxartwork saves data to the database.
		 *
		 * @since 1.0.0
		 *
		 ...
		 */
		do_action( 'madxartwork/editor/after_save', $this->post->ID, $editor_data );
	}
	...
	public function save_version() {
		if ( ! defined( 'IS_madxartwork_UPGRADE' ) ) {
			// Save per revision.
			$this->update_meta( '_madxartwork_version', madxartwork_VERSION );

			/**
			 * Document version save.
			 *
			 * Fires when document version is saved on madxartwork.
			 * Will not fire during madxartwork Upgrade.
			 *
			 * @since 2.5.12
			 *
			 * @param \madxartwork\Core\Base\Document $this The current document.
			 *
			 */
			do_action( 'madxartwork/document/save_version', $this );
		}
	}
	...
	public function get_last_edited() {
		...
		$date = date_i18n( _x( 'M j, H:i', 'revision date format', 'madxartwork' ), strtotime( $post->post_modified ) );
		$display_name = get_the_author_meta( 'display_name', $post->post_author );

		if ( $autosave_post || 'revision' === $post->post_type ) {
			/* translators: 1: Saving date, 2: Author display name */
			$last_edited = sprintf( __( 'Draft saved on %1$s by %2$s', 'madxartwork' ), '<time>' . $date . '</time>', $display_name );
		} else {
			/* translators: 1: Editing date, 2: Author display name */
			$last_edited = sprintf( __( 'Last edited on %1$s by %2$s', 'madxartwork' ), '<time>' . $date . '</time>', $display_name );
		}
	...
	public function __construct( array $data = [] ) {
		if ( $data ) {
			...
			$saved_settings = get_post_meta( $this->post->ID, '_madxartwork_page_settings', true );
