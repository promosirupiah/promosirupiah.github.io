<?php
namespace madxartwork\Core\Common\Modules\Ajax;
...
/**
 * madxartwork ajax manager.
 *
 * madxartwork ajax manager handler class is responsible for handling madxartwork
...
class Module extends BaseModule {
	...
	const NONCE_KEY = 'madxartwork_ajax';
	 ...
	 *
	 * Initializing madxartwork ajax manager.
	 *
	 * @since 2.0.0
	 * @access public
	 */
	public function __construct() {
		add_action( 'wp_ajax_madxartwork_ajax', [ $this, 'handle_ajax_request' ] );
	}
	...
	public function register_ajax_action( $tag, $callback ) {
		if ( ! did_action( 'madxartwork/ajax/register_actions' ) ) {
			_doing_it_wrong( __METHOD__, esc_html( sprintf( 'Use `%s` hook to register ajax action.', 'madxartwork/ajax/register_actions' ) ), '2.0.0' );
		}
	 ...
	 *
	 * Fired by `wp_ajax_madxartwork_ajax` action.
	 *
	 * @since 2.0.0
	 * @access public
	 */
	public function handle_ajax_request() {
		if ( ! $this->verify_request_nonce() ) {
			$this->add_response_data( false, __( 'Token Expired.', 'madxartwork' ) )
		 ...
		 */
		do_action( 'madxartwork/ajax/register_actions', $this );
		...
		foreach ( $this->requests as $id => $action_data ) {
			...
			if ( ! isset( $this->ajax_actions[ $action_data['action'] ] ) ) {
				$this->add_response_data( false, __( 'Action not found.', 'madxartwork' ), Exceptions::BAD_REQUEST );
	 ...
	 * give the user an access to madxartwork ajax actions.
	 *
	 * @since 2.3.0
	 * @access public
	 *
