<?php
namespace madxartwork\Core\Files\Assets\Svg;
...
class Svg_Handler {
	 ...
	 */
	const META_KEY = '_madxartwork_inline_svg';
	...
	public static function is_svg_uploads_enabled() {
		return ! ! get_option( 'madxartwork_allow_svg', false );
	}
	...
	public function upload_mimes( $allowed_types ) {
		if ( $this->is_madxartwork_media_upload() ) {
	...
	public function wp_handle_upload_prefilter( $file ) {
		if ( ! $this->is_madxartwork_media_upload() || self::MIME_TYPE !== $file['type'] ) {
			return $file;
		}
		...
		if ( 'svg' !== $ext ) {
			$file['error'] = sprintf( __( 'The uploaded %s file is not supported. Please upload a valid SVG file', 'madxartwork' ), $ext );
			return $file;
		}

		if ( ! self::is_enabled() ) {
			$file['error'] = __( 'SVG file is not allowed for security reasons', 'madxartwork' );
			return $file;
		}

		if ( self::svg_sanitizer_can_run() && ! $this->sanitize_svg( $file['tmp_name'] ) ) {
			$file['error'] = __( 'Invalid SVG Format, file not uploaded for security reasons', 'madxartwork' );
		}
	...
	/**
	 * is_madxartwork_media_upload
	 * @return bool
	 */
	private function is_madxartwork_media_upload() {
		return isset( $_POST['uploadTypeCaller'] ) && 'madxartwork-editor-upload' === $_POST['uploadTypeCaller']; // phpcs:ignore
	}
	...
	private function get_allowed_attributes() {
		...
		return apply_filters( 'madxartwork/files/svg/allowed_attributes', $allowed_attributes );
	}
	...
	private function get_allowed_elements() {
		...
		return apply_filters( 'madxartwork/files/svg/allowed_elements', $allowed_elements );
	}
	...
	public static function is_enabled() {
		...
		if ( null === $enabled ) {
			...
			$enabled = apply_filters( 'madxartwork/files/svg/enabled', $enabled );
		}
		return $enabled;
	}
	...
	public function __construct() {
		...
		add_action( 'madxartwork/core/files/clear_cache', [ $this, 'delete_meta_cache' ] );
	}
}
