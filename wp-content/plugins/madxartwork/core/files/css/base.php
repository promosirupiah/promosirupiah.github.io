...
use madxartwork\Core\Files\Base as Base_File;
...
/**
 * madxartwork CSS file.
 *
 * madxartwork CSS file handler class is responsible for generating CSS files.
 *
 * @since 1.2.0
 * @abstract
 */
abstract class Base extends Base_File {
	...
	/**
	 * madxartwork CSS file generated status.
	 *
	 ...
	 *
	 * Initializing madxartwork CSS file.
	 *
	 * @since 1.2.0
	 * @access public
	 */
	...
	protected function use_external_file() {
		return 'internal' !== get_option( 'madxartwork_css_print_method' );
	}
	 ...
	 * Enqueue CSS.
	 *
	 * Either enqueue the CSS file in madxartwork or add inline style.
	 *
	...
	public function enqueue() {
		 ...
		 * Enqueue CSS file.
		 *
		 * Fires when CSS file is enqueued on madxartwork.
		 *
		 * The dynamic portion of the hook name, `$name`, refers to the CSS file name.
		 *
		 * @since 1.9.0
		 * @deprecated 2.0.0 Use `madxartwork/css-file/{$name}/enqueue` action instead.
		 *
		 * @param Base $this The current CSS file.
		 */
		do_action_deprecated( "madxartwork/{$name}-css-file/enqueue", [ $this ], '2.0.0', "madxartwork/css-file/{$name}/enqueue" );
		 ...
		 * @param Base $this The current CSS file.
		 */
		do_action( "madxartwork/css-file/{$name}/enqueue", $this );
	}
	...
	public function add_controls_stack_style_rules( Controls_Stack $controls_stack, array $controls, array $values, array $placeholders, array $replacements, array $all_controls = null ) {
		...
		foreach ( $controls as $control ) {
			...
			if ( ! empty( $parsed_dynamic_settings[ Manager::DYNAMIC_SETTING_KEY ][ $control['name'] ] ) ) {
				...
				// Instead it's handled by \madxartwork\Core\DynamicTags\Dynamic_CSS
	...
	protected function parse_content() {
		 ...
		 * Parse CSS file.
		 *
		 * Fires when CSS file is parsed on madxartwork.
		 *
		 * The dynamic portion of the hook name, `$name`, refers to the CSS file name.
		 *
		 * @since 1.2.0
		 * @deprecated 2.0.0 Use `madxartwork/css-file/{$name}/parse` action instead.
		 *
		 * @param Base $this The current CSS file.
		 */
		do_action_deprecated( "madxartwork/{$name}-css-file/parse", [ $this ], '2.0.0', "madxartwork/css-file/{$name}/parse" );
		 ...
		 * @param Base $this The current CSS file.
		 */
		do_action( "madxartwork/css-file/{$name}/parse", $this );
	...
	protected function add_repeater_control_style_rules( Controls_Stack $controls_stack, array $repeater_control, array $repeater_values, array $placeholders, array $replacements ) {
		...
		foreach ( $repeater_control['style_fields'] as $index => $item ) {
			$this->add_controls_stack_style_rules(
				...
				$placeholders,
				array_merge( $replacements, [ '.madxartwork-repeater-item-' . $repeater_values[ $index ]['_id'] ] ),
	...
	protected function add_dynamic_control_style_rules( array $control, $value ) {
		Plugin::$instance->dynamic_tags->parse_tags_text( $value, $control, function( $id, $name, $settings ) {
			...
			$this->add_controls_stack_style_rules( $tag, $tag->get_style_controls(), $tag->get_active_settings(), [ '{{WRAPPER}}' ], [ '#madxartwork-tag-' . $id ] );
		} );
	}
}
