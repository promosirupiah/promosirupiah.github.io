...
/**
 * madxartwork post CSS file.
 *
 * madxartwork CSS file handler class is responsible for generating the single
 * post CSS file.
 *
 * @since 1.2.0
 */
class Post extends Base {
	...
	/**
	 * madxartwork post CSS file prefix.
	 */
	const FILE_PREFIX = 'post-';

	const META_KEY = '_madxartwork_css';
	...
	public function get_element_unique_selector( Element_Base $element ) {
		return '.madxartwork-' . $this->post_id . ' .madxartwork-element' . $element->get_unique_selector();
	}
	 ...
	 * Enqueue CSS.
	 *
	 * Enqueue the post CSS file in madxartwork.
	 *
	 * This method ensures that the post was actually built with madxartwork before
	...
	public function enqueue() {
		if ( ! Plugin::$instance->db->is_built_with_madxartwork( $this->post_id ) ) {
			return;
		}
	...
	protected function get_file_handle_id() {
		return 'madxartwork-post-' . $this->post_id;
	}
	...
	protected function render_styles( Element_Base $element ) {
		 ...
		 * @param Element_Base $element The element.
		 */
		do_action( 'madxartwork/element/before_parse_css', $this, $element );
		 ...
		 * @param Element_Base $element The element.
		 */
		do_action( 'madxartwork/element/parse_css', $this, $element );
	}
}
