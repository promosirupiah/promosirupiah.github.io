<?php
namespace madxartwork\Core\Settings\Page;
...
/**
 * madxartwork page settings model.
 *
 * madxartwork page settings model handler class is responsible for registering
 * and managing madxartwork page settings models.
 *
 * @since 1.6.0
 */
class Model extends BaseModel {
	 ...
	 * Model constructor.
	 *
	 * Initializing madxartwork page settings model.
	 *
	 * @since 1.6.0
	 * @access public
	 *
	...
	public function get_panel_page_settings() {
		...
		return [
			...
			'title' => sprintf( __( '%s Settings', 'madxartwork' ), $document::get_title() ),
		];
	}
	 ...
	 * exclude it from the exported madxartwork data.
	 *
	 * @since 1.6.0
	 * @access public
	 *
	...
	public function on_export( $element_data ) {
		if ( ! empty( $element_data['settings']['template'] ) ) {
			/**
			 * @var \madxartwork\Modules\PageTemplates\Module $page_templates_module
			 */
			$page_templates_module = Plugin::$instance->modules_manager->get_modules( 'page-templates' );
			$is_madxartwork_template = ! ! $page_templates_module->get_template_path( $element_data['settings']['template'] );

			if ( ! $is_madxartwork_template ) {
