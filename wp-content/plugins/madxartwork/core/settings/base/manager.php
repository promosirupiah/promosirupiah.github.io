...
/**
 * madxartwork settings base manager.
 *
 * madxartwork settings base manager handler class is responsible for registering
 * and managing madxartwork settings base managers.
 *
 * @since 1.6.0
 * @abstract
 */
abstract class Manager {
	 ...
	 *
	 * Initializing madxartwork settings base manager.
	 *
	 * @since 1.6.0
	 * @access public
	 */
	public function __construct() {
		...
		add_action( "madxartwork/css-file/{$name}/parse", [ $this, 'add_settings_css_rules' ] );
	}
	 ...
	 *
	 * Fired by `madxartwork/ajax/register_actions` action.
	 *
	 * @since 2.0.0
	 * @access public
	 *
	...
	final public function ajax_save_settings( $request ) {
		 ...
		 *
		 * @since 1.6.0
		 * @deprecated 2.0.0 Use `madxartwork/settings/{$settings_name}/success_response_data` filter.
		 *
		 * @param array $success_response_data Success response data.
		 * @param int   $id                    Settings ID.
		 * @param array $data                  Settings data.
		 */
		$success_response_data = apply_filters_deprecated( "madxartwork/{$settings_name}/settings/success_response_data", [ $success_response_data, $id, $data ], '2.0.0', "madxartwork/settings/{$settings_name}/success_response_data" );
		 ...
		 * @param array $data                  Settings data.
		 */
		$success_response_data = apply_filters( "madxartwork/settings/{$settings_name}/success_response_data", $success_response_data, $id, $data );
	 ...
	 *
	 * Fired by `madxartwork/css-file/{$name}/parse` action.
	 *
	 * @since 1.6.0
	 * @access public
	 *
	 ...
	 *
	 * Fired by `madxartwork/init` action.
	 *
	 * @since 2.3.0
	 * @access public
	 */
	...
	protected function print_editor_template_content( $name ) {
		...
		<div class="madxartwork-panel-navigation">
			<# _.each( madxartwork.config.settings.<?php echo esc_html( $name ); ?>.tabs, function( tabTitle, tabSlug ) {
		...
		<div id="madxartwork-panel-<?php echo esc_attr( $name ); ?>-settings-controls"></div>
		<?php
	}
	...
	private function get_editor_template() {
		...
		ob_start();
		?>
		<script type="text/template" id="tmpl-madxartwork-panel-<?php echo esc_attr( $name ); ?>-settings">
