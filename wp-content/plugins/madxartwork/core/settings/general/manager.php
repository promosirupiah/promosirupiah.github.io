...
use madxartwork\Core\Settings\Base\Manager as BaseManager;
...
/**
 * madxartwork general settings manager.
 *
 * madxartwork general settings manager handler class is responsible for registering
 * and managing madxartwork general settings managers.
 *
 * @since 1.6.0
 */
class Manager extends BaseManager {
	 ...
	 */
	const META_KEY = '_madxartwork_general_settings';

	/**
	 * General settings manager constructor.
	 *
	 * Initializing madxartwork general settings manager.
	 *
	 * @since 1.6.0
	 * @access public
	 */
			...
			foreach ( $sections as $section_name => $section_data ) {
				...
				foreach ( $section_data['controls'] as $control_name => $control_data ) {
					if ( isset( $settings[ $control_name ] ) ) {
						$one_list_control_name = str_replace( 'madxartwork_', '', $control_name );
	...
	private function add_panel_tabs() {
		Controls_Manager::add_tab( self::PANEL_TAB_LIGHTBOX, __( 'Lightbox', 'madxartwork' ) );
	}
}
