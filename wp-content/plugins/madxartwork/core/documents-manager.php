...
/**
 * madxartwork documents manager.
 *
 * madxartwork documents manager handler class is responsible for registering and
 * managing madxartwork documents.
 *
 * @since 2.0.0
 */
class Documents_Manager {
	 ...
	 *
	 * Initializing the madxartwork documents manager.
	 *
	 * @since 2.0.0
	 * @access public
	 */
	public function __construct() {
		add_action( 'madxartwork/documents/register', [ $this, 'register_default_types' ], 0 );
	...
	public function get( $post_id, $from_cache = true ) {
		...
		$post_id = apply_filters( 'madxartwork/documents/get/post_id', $post_id );
	...
	public function create( $type, $post_data = [], $meta_data = [] ) {
		...
		if ( empty( $post_data['post_title'] ) ) {
			$post_data['post_title'] = __( 'madxartwork', 'madxartwork' );
			if ( 'post' !== $type ) {
				$post_data['post_title'] = sprintf(
					/* translators: %s: Document title */
					__( 'madxartwork %s', 'madxartwork' ),
		...
		$meta_data['_madxartwork_edit_mode'] = 'builder';
	 ...
	 *
	 * @throws \Exception If current user don't have permissions to edit the post or the post is not using madxartwork.
	 *
	...
	public function ajax_save( $request ) {
		...
		if ( ! $document->is_built_with_madxartwork() || ! $document->is_editable_by_current_user() ) {
		 ...
		 */
		$return_data = apply_filters( 'madxartwork/documents/ajax_save/return_data', $return_data, $document );
	...
	private function register_types() {
		if ( ! did_action( 'madxartwork/documents/register' ) ) {
			/**
			 * Register madxartwork documents.
			 *
			 * @since 2.0.0
			 *
			 * @param Documents_Manager $this The document manager instance.
			 */
			do_action( 'madxartwork/documents/register', $this );
		}
	}
}
