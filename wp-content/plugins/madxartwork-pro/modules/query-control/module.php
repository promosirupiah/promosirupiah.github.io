<?php
namespace madxartworkPro\Modules\QueryControl;
...
use madxartworkPro\Modules\QueryControl\Classes\madxartwork_Related_Query;
use madxartworkPro\Modules\QueryControl\Controls\Query;
...
class Module extends Module_Base {
	...
	public static function add_exclude_controls( $widget ) {
		...
		$widget->add_control(
			...
			[
				...
				'default' => '',
				'description' => __( 'Set to Yes to avoid duplicate posts from showing up on the page. This only affects the frontend.', 'madxartwork-pro' ),
			]
		);
	 ...
	 *                            `madxartwork/query/[get_value_titles|get_autocomplete]/{user_defined}`.
	 *
	 ...
	 *      user_defined        : will invoke apply_filters using the user_defined value as filter name,
	 *                            `madxartwork/query/[get_value_titles|get_autocomplete]/display/{user_defined}`
	 *
	...
	public function ajax_posts_filter_autocomplete_deprecated( $data ) {
		...
		switch ( $data['filter_type'] ) {
			...
			case 'author':
				...
				break;
			default:
				$results = apply_filters( 'madxartwork_pro/query_control/get_autocomplete/' . $data['filter_type'], [], $data );
		}
	...
	public function ajax_posts_filter_autocomplete( array $data ) {
		...
		switch ( $query_data['object'] ) {
			case self::QUERY_OBJECT_TAX:
				...
				foreach ( $terms as $term ) {
					if ( apply_filters( "madxartwork/query/get_autocomplete/tax/{$display}", true, $term, $data ) ) {
						$results[] = [
							'id' => $term->{$by_field},
			...
			case self::QUERY_OBJECT_POST:
				...
				foreach ( $query->posts as $post ) {
					if ( apply_filters( "madxartwork/query/get_autocomplete/custom/{$display}", true, $post, $data ) ) {
			...
			case self::QUERY_OBJECT_AUTHOR:
				...
				foreach ( $user_query->get_results() as $user ) {
					if ( apply_filters( "madxartwork/query/get_autocomplete/user/{$display}", true, $user, $data ) ) {
						$results[] = [
							'id' => $user->ID,
				...
				break;
			default:
				$results = apply_filters( 'madxartwork/query/get_autocomplete/' . $query_data['filter_type'], $results, $data );
		}
	...
	public function ajax_posts_control_value_titles_deprecated( $request ) {
		...
		switch ( $request['filter_type'] ) {
			...
			case 'author':
				...
				break;
			default:
				$results = apply_filters( 'madxartwork_pro/query_control/get_value_titles/' . $request['filter_type'], [], $request );
		}
	...
	public function ajax_posts_control_value_titles( $request ) {
		...
		switch ( $query_data['object'] ) {
			case self::QUERY_OBJECT_TAX:
				...
				foreach ( $terms as $term ) {
					if ( apply_filters( "madxartwork/query/get_value_titles/tax/{$display}", true, $term, $request ) ) {
			...
			case self::QUERY_OBJECT_POST:
				...
				foreach ( $query->posts as $post ) {
					if ( apply_filters( "madxartwork/query/get_value_titles/custom/{$display}", true, $post, $request ) ) {
			...
			case self::QUERY_OBJECT_USER:
				...
				foreach ( $user_query->get_results() as $user ) {
					if ( apply_filters( "madxartwork/query/get_value_titles/user/{$display}", true, $user, $request ) ) {
						$results[ $user->ID ] = $this->format_user_for_display( $user, $display, $request, 'get_value_titles' );
					}
				}
				break;
			default:
				$results = apply_filters( "madxartwork/query/get_value_titles/{$query_data['filter_type']}", $results, $request );
		}
	...
	private function get_term_name( $term, $display, $request, $filter_name = 'get_autocomplete' ) {
		...
		switch ( $display ) {
			...
			case 'minimal':
				...
				break;
			default:
				$text = apply_filters( "madxartwork/query/{$filter_name}/display/{$display}", $term_name, $request );
				break;
		}
		return $text;
	}
	...
	private function format_post_for_display( $post, $display, $data, $filter_name = 'get_autocomplete' ) {
		...
		switch ( $display ) {
			...
			case 'detailed':
				...
				break;
			default:
				$text = apply_filters( "madxartwork/query/{$filter_name}/display/{$display}", $post->post_title, $post->ID, $data );
				break;
		}
	...
	private function format_user_for_display( $user, $display, $data, $filter_name = 'get_autocomplete' ) {
		switch ( $display ) {
			...
			case 'detailed':
				...
				break;
			default:
				$text = apply_filters( "madxartwork/query/{$filter_name}/display/{$display}", $user, $data );
				break;
		}
	...
	/**
	 * @deprecated use madxartwork_Post_Query capabilities
	 *
	...
	public function get_query_args( $control_id, $settings ) {
		// TODO: _deprecated_function( __METHOD__, '2.5.0', 'class madxartwork_Post_Query' );
	...
	/**
	 * @param \madxartworkPro\Base\Base_Widget $widget
	 * @param string $name
	...
	public function get_query( $widget, $name, $query_args = [], $fallback_args = [] ) {
		...
		if ( 'related' === $post_type ) {
			$madxartwork_query = new madxartwork_Related_Query( $widget, $name, $query_args, $fallback_args );
		} else {
			$madxartwork_query = new madxartwork_Post_Query( $widget, $name, $query_args );
		}
		return $madxartwork_query->get_query();
	}
