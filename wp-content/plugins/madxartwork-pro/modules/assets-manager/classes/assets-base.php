<?php
namespace madxartworkPro\Modules\AssetsManager\Classes;
...
abstract class Assets_Base {
	...
	public function print_metabox( $fields ) {
		?>
		<div class="madxartwork-metabox-content">
			<?php
	...
	public function get_field_label( $field ) {
		...
		return '<p class="madxartwork-field-label"><label for="' . esc_attr( $id ) . '">' . $field['label'] . '</label></p>';
	}
	...
	public function get_file_field( $field, $saved ) {
		...
		$html .= $this->get_input_field(
			[
				'type' => 'button',
				'class' => 'button madxartwork-button madxartwork-upload-btn',
				'name' => $field['id'],
				'id' => $field['id'],
				'value' => '',
				...
				'data-upload_text' => __( 'Upload', 'madxartwork-pro' ),
				'data-remove_text' => __( 'Delete', 'madxartwork-pro' ),
	...
	public function get_dropzone_field( $field ) {
		...
		$field['label'] = '<h4><span class="box__dragndrop">' . __( 'Drag & Drop to Upload', 'madxartwork-pro' ) . '</span></h4>';
		if ( ! empty( $field['sub-label'] ) ) {
			$field['label'] .= '<h5>' . $field['sub-label'] . '</h5>';
		}
		?>
		<div class="madxartwork-dropzone-field">
			<div class="box__input">
				<div class="madxartwork--dropzone--upload__icon">
					<i class="eicon-library-upload"></i>
				</div>
				<?php echo $input_html; ?>
				<?php echo $this->get_field_label( $field ); ?>
				<div class="madxartwork-button madxartwork--dropzone--upload__browse">
					<span><?php esc_html_e( 'Click here to browse', 'madxartwork-pro' ); ?></span>
				</div>
			</div>
			<div class="box__uploading"><?php esc_html_e( 'Uploading&hellip;', 'madxartwork-pro' ); ?></div>
			<div class="box__success"><?php esc_html_e( 'Done!', 'madxartwork-pro' ); ?></div>
			<div class="box__error"><?php esc_html_e( 'Error!', 'madxartwork-pro' ); ?> <span></span>.</div>
		</div>
		<?php
		return ob_get_clean();
	}
	...
	public function get_repeater_field( $field, $saved ) {
		...
		$add_label = isset( $field['add_label'] ) ? $field['add_label'] : __( 'Add item', 'madxartwork-pro' );
		$row_label = isset( $field['row_label'] ) ? $field['row_label'] : __( 'Row', 'madxartwork-pro' );
		$row_label_html_args = [
			'id' => 'row_label_' . $js_id,
		...
		echo '<input type="button" class="button madxartwork-button add-repeater-row" value="' . esc_attr( $add_label ) . '" data-template-id="' . $js_id . '_block">';
	...
	private function get_repeater_tools( $field ) {
		$confirm = isset( $field['confirm'] ) ? $field['confirm'] : __( 'Are you sure?', 'madxartwork-pro' );
		$remove_title = isset( $field['remove_title'] ) ? $field['remove_title'] : __( 'Delete', 'madxartwork-pro' );
		$toggle_title = isset( $field['toggle_title'] ) ? $field['toggle_title'] : __( 'Edit', 'madxartwork-pro' );
		$close_title = isset( $field['close_title'] ) ? $field['close_title'] : __( 'Close', 'madxartwork-pro' );

		return '<span class="madxartwork-repeater-tool-btn close-repeater-row" title="' . esc_attr( $close_title ) . '">
                    <i class="eicon-times" aria-hidden="true"></i>' . $close_title . '
                </span>
                <span class="madxartwork-repeater-tool-btn toggle-repeater-row" title="' . esc_attr( $toggle_title ) . '">
                    <i class="eicon-edit" aria-hidden="true"></i>' . $toggle_title . '
                </span>
                <span class="madxartwork-repeater-tool-btn remove-repeater-row" data-confirm="' . $confirm . '" title="' . esc_attr( $remove_title ) . '">
	...
	public function get_field_row( $field, $field_html ) {
		...
		$css_id .= ' madxartwork-field-' . $field['field_type'];

		return '<div class="madxartwork-field' . $css_id . '">' . $this->get_field_label( $field ) . $field_html . $description . '</div>';
	}
