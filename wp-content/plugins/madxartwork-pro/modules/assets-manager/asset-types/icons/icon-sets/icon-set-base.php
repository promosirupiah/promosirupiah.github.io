...
use madxartworkPro\Modules\AssetsManager\AssetTypes\Icons\Custom_Icons;
...
abstract class Icon_Set_Base {
	...
	protected function get_file_url( $file_name ) {
		...
		$url = $wp_upload_dir['baseurl'] . '/madxartwork/custom-icons/' . $file_name;
		 ...
		 */
		$url = apply_filters( 'madxartwork_pro/icons_manager/custom_icons/url', $url, $file_name );
	...
	protected function get_icon_sets_dir() {
		$wp_upload_dir = wp_upload_dir();
		$path = $wp_upload_dir['basedir'] . '/madxartwork/custom-icons';
		 ...
		 * @param string $path .
		 */
		$path = apply_filters( 'madxartwork_pro/icons_manager/custom_icons/dir', $path );
	...
	public function move_files( $post_id ) {
		...
		update_post_meta( $post_id, '_madxartwork_icon_set_path', $to );
