<?php
namespace madxartworkPro\Modules\AssetsManager\AssetTypes\Icons;

use madxartworkPro\Modules\AssetsManager\Classes\Assets_Base;
use madxartworkPro\Modules\AssetsManager\AssetTypes\Icons_Manager;
...
class Custom_Icons extends  Assets_Base {
	...
	const META_KEY = 'madxartwork_custom_icon_set_config';
	const OPTION_NAME = 'madxartwork_custom_icon_sets_config';

	public $current_post_id = 0;

	public function get_name() {
		return __( 'Custom Icons', 'madxartwork-pro' );
	}
	...
	public function add_meta_box() {
		add_meta_box(
			'madxartwork-custom-icons-metabox',
			__( 'Icon Set', 'madxartwork-pro' ),
			[ $this, 'render_metabox' ],
			Icons_Manager::CPT,
			'normal',
			'default'
	...
	public function render_metabox( $post ) {
		...
		$fields = [
			[
				...
				'tag' => 'div',
				'attributes' => [
					'class' => 'madxartwork-custom-icons-metabox',
				],
			],
			[
				'id' => 'zip_upload',
				...
				'label' => false,
				'sub-label' => __( 'Your Fontello, IcoMoon or Fontastic .zip file', 'madxartwork-pro' ),
			],
			[
				'id' => 'close_div',
				'field_type' => 'html_tag',
	...
	public static function get_supported_icon_sets() {
		...
		return array_merge( apply_filters( 'madxartwork_pro/icons_manager/custom_icons/additional_supported_types', [] ), $icon_sets );
	}
	...
	private function upload() {
		...
		if ( 'zip' !== $ext ) {
			...
			return new \WP_Error( 'unsupported_file', __( 'Only zip files are allowed', 'madxartwork-pro' ) );
		}
	...
	private function upload_and_extract_zip() {
		...
		if ( count( $source_files ) === 0 ) {
			return new \WP_Error( 'incompatible_archive', esc_html__( 'Incompatible archive', 'madxartwork-pro' ) );
		}
	...
	public function custom_icons_upload_handler( $data ) {
		...
		return new \WP_Error( 'unsupported_zip_format', __( 'The zip file provided is not supported!', 'madxartwork-pro' ) );
	}
	...
	public function handle_delete_icon_set( $post_id ) {
		...
		$icon_set_dir = get_post_meta( $post_id, '_madxartwork_icon_set_path', true );
	...
	public function manage_columns( $columns ) {
		return [
			...
			'title' => __( 'Icon Set', 'madxartwork-pro' ),
			'icons_prefix' => __( 'CSS Prefix', 'madxartwork-pro' ),
		];
	}

	public function update_enter_title_here( $title, $post ) {
		if ( isset( $post->post_type ) && Icons_Manager::CPT === $post->post_type ) {
			return __( 'Enter Icon Set Name', 'madxartwork-pro' );
		}
	...
	public function add_custom_icon_templates( $current_screen ) {
		if ( 'madxartwork_icons' !== $current_screen->id || 'post' !== $current_screen->base ) {
			return;
		}
		Plugin::madxartwork()->common->add_template( __DIR__ . '/templates.php' );
	}
	...
	protected function actions() {
		...
		add_filter( 'madxartwork/icons_manager/additional_tabs', [ $this, 'register_icon_libraries_control' ] );
		add_filter( 'madxartwork/editor/localize_settings', [ $this, 'add_custom_icons_url' ] );
