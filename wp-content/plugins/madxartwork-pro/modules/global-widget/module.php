<?php
namespace madxartworkPro\Modules\GlobalWidget;
...
class Module extends Module_Base {
	...
	const WIDGET_TYPE_META_KEY = '_madxartwork_template_widget_type';

	const INCLUDED_POSTS_LIST_META_KEY = '_madxartwork_global_widget_included_posts';
	...
	public function add_templates_localize_data( $settings ) {
		...
		$templates_manager = $madxartwork->templates_manager;

		$widgets_types = $madxartwork->widgets_manager->get_widget_types();
		...
		$settings = array_replace_recursive( $settings, [
			...
			'i18n' => [
				'unlink' => __( 'Unlink', 'madxartwork-pro' ),
				'cancel' => __( 'Cancel', 'madxartwork-pro' ),
				'unlink_widget' => __( 'Unlink Widget', 'madxartwork-pro' ),
				'global' => __( 'Global', 'madxartwork-pro' ),
				'dialog_confirm_unlink' => __( 'This will make the widget stop being global. It\'ll be reverted into being just a regular widget.', 'madxartwork-pro' ),
				'global_widget_save_title' => __( 'Save your widget as a global widget', 'madxartwork-pro' ),
				'global_widget_save_description' => __( 'You\'ll be able to add this global widget to multiple areas on your site, and edit it from one single place.', 'madxartwork-pro' ),
				'linked_to_global' => __( 'Linked to Global', 'madxartwork-pro' ),
			],
		] );
	...
	public function is_post_type_support_madxartwork( $is_supported, $post_id, $post_type ) {
		...
		if ( $is_widget_template && function_exists( 'get_current_screen' ) ) {
			...
			if ( ! empty( $screen->id ) && in_array( $screen->id, [ 'madxartwork_library', 'edit-madxartwork_library' ] ) ) {
				$is_supported = false;
			}
		}
	...
	/**
	 * TODO: Remove. On madxartwork 2.3.3 it's handled by the Documents Manager.
	 *
	...
	public function set_global_widget_included_posts_list( $post_id, $editor_data ) {
		...
		Plugin::madxartwork()->db->iterate_data( $editor_data, function( $element_data ) use ( & $global_widget_ids ) {
			if ( isset( $element_data['templateID'] ) ) {
	...
	private function delete_included_posts_css( $template_id ) {
		...
		foreach ( array_keys( $including_post_ids ) as $post_id ) {
			delete_post_meta( $post_id, '_madxartwork_css' );
		}
	}
	...
	private function add_hooks() {
		...
		add_action( 'madxartwork/template-library/after_save_template', [ $this, 'set_template_widget_type_meta' ], 10, 2 );
		add_action( 'madxartwork/template-library/after_update_template', [ $this, 'on_template_update' ], 10, 2 );
		add_action( 'madxartwork/editor/init', [ $this, 'on_madxartwork_editor_init' ] );
		add_action( 'madxartwork/editor/after_save', [ $this, 'set_global_widget_included_posts_list' ], 10, 2 );

		add_filter( 'madxartwork_pro/editor/localize_settings', [ $this, 'add_templates_localize_data' ] );
		add_filter( 'madxartwork/template-library/get_template', [ $this, 'filter_template_data' ] );
		add_filter( 'madxartwork/element/get_child_type', [ $this, 'get_element_child_type' ], 10, 2 );
		add_filter( 'madxartwork/utils/is_post_support', [ $this, 'is_post_type_support_madxartwork' ], 10, 3 );
		add_filter( 'user_has_cap', [ $this, 'remove_user_edit_cap' ], 10, 3 );

		add_filter( 'madxartwork/template_library/is_template_supports_export', [ $this, 'is_template_supports_export' ], 10, 2 );
	}
}
