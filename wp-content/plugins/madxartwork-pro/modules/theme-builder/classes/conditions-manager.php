...
use madxartworkPro\Modules\ThemeBuilder\Conditions\Condition_Base;
...
class Conditions_Manager {
	...
	public function admin_columns_headers( $posts_columns ) {
		...
		$posts_columns = array_slice( $posts_columns, 0, $offset, true ) + [
			'instances' => __( 'Instances', 'madxartwork-pro' ),
	...
	public function ajax_check_conditions_conflicts( $request ) {
		...
		/** @var \madxartworkPro\Modules\ThemeBuilder\Module $theme_builder_module */
		$theme_builder_module = Module::instance();
		...
		if ( ! empty( $conflicted ) ) {
			$message = __( 'madxartwork recognized that you have set this location for other templates: ', 'madxartwork-pro' ) . ' ' . implode( ', ', $conflicted );
		}
	...
	private function register_condition( $id, $args = [] ) {
		...
		$class_name = '\\madxartworkPro\\Modules\\ThemeBuilder\\Conditions\\' . $class_name;
	...
	public function register_conditions() {
		...
		do_action( 'madxartwork/theme/register_conditions', $this );
	}
	...
	public function save_conditions( $post_id, $conditions ) {
		...
		if ( empty( $conditions_to_save ) ) {
			...
			delete_post_meta( $post_id, '_madxartwork_conditions' );
		} else {
			$document->update_meta( '_madxartwork_conditions', $conditions_to_save );
		}
	...
	public function get_location_templates( $location ) {
		...
		foreach ( $conditions_groups as $theme_template_id => $conditions ) {
			$theme_template_id = apply_filters( 'madxartwork/theme/get_location_templates/template_id', $theme_template_id );
			...
			foreach ( $conditions as $condition ) {
				...
				if ( $condition_pass && $sub_name ) {
					...
					$args = [
						'id' => apply_filters( 'madxartwork/theme/get_location_templates/condition_sub_id', $sub_id, $parsed_condition ),
					];
	...
	public function get_document_conditions( $document ) {
		$saved_conditions = $document->get_main_meta( '_madxartwork_conditions' );
		$conditions = [];
