...
class Single extends Theme_Page_Document {
	 ...
	 */
	const REMOTE_CATEGORY_META_KEY = '_madxartwork_template_sub_type';
	...
	public static function get_title() {
		return __( 'Single', 'madxartwork-pro' );
	}
	...
	protected static function get_editor_panel_categories() {
		$categories = [
			'theme-elements-single' => [
				'title' => __( 'Single', 'madxartwork-pro' ),
			],
		];
	...
	public function print_content() {
		...
		if ( $requested_post_id !== $this->post->ID ) {
			...
			if ( $requested_document && ! $requested_document instanceof Section && $requested_document->get_location() !== $this->get_location() ) {
				echo '<div class="madxartwork-theme-builder-content-area">' . __( 'Content Area', 'madxartwork-pro' ) . '</div>';
	...
	public static function get_preview_as_options() {
		...
		return [
			'single' => [
				'label' => __( 'Single', 'madxartwork-pro' ),
				'options' => $post_types_options,
			],
			'page/404' => __( '404', 'madxartwork-pro' ),
		];
	}

	public function get_depended_widget() {
		return Plugin::madxartwork()->widgets_manager->get_widget_types( 'theme-post-content' );
	}

	public function get_elements_data( $status = DB::STATUS_PUBLISH ) {
		$data = parent::get_elements_data();

		if ( Plugin::madxartwork()->preview->is_preview_mode() && self::get_property( 'location' ) === Module::instance()->get_locations_manager()->get_current_location() ) {
			$has_the_content = false;

			$depended_widget = $this->get_depended_widget();

			Plugin::madxartwork()->db->iterate_data( $data, function( $element ) use ( &$has_the_content, $depended_widget ) {
	...
	public function preview_error_handler() {
		...
		wp_localize_script( 'madxartwork-frontend', 'madxartworkPreviewErrorArgs', [
			/* translators: %s: is the widget name. */
			'headerMessage' => sprintf( __( 'The %s Widget was not found in your template.', 'madxartwork-pro' ), $depended_widget_title ),
			/* translators: %1$s: is the widget name. %2$s: is the template name.  */
			'message' => sprintf( __( 'You must include the %1$s Widget in your template (%2$s), in order for madxartwork to work on this page.', 'madxartwork-pro' ), $depended_widget_title, '<strong>' . static::get_title() . '</strong>' ),
			'strings' => [
				'confirm' => __( 'Edit Template', 'madxartwork-pro' ),
			],
