...
abstract class Theme_Document extends Library_Document {
	...
	const LOCATION_META_KEY = '_madxartwork_location';
	...
	public function get_location_label() {
		...
		if ( ! $supported ) {
			$label .= ' (' . __( 'Unsupported', 'madxartwork-pro' ) . ')';
		}
	...
	public function get_container_attributes() {
		...
		if ( $location ) {
			$attributes['class'] .= ' madxartwork-location-' . $location;
		}
	...
	public function get_edit_url() {
		...
		if ( isset( $_GET['action'] ) && 'madxartwork_new_post' === $_GET['action'] ) {
			$url .= '#library';
		}
	...
	protected function _register_controls() {
		...
		$this->start_controls_section(
			'preview_settings',
			[
				'label' => __( 'Preview Settings', 'madxartwork-pro' ),
		...
		$this->add_control(
			'preview_type',
			[
				'label' => __( 'Preview Dynamic Content as', 'madxartwork-pro' ),
				'label_block' => true,
		...
		$this->add_control(
			'preview_search_term',
			[
				'label' => __( 'Search Term', 'madxartwork-pro' ),
				'export' => false,
				'condition' => [
					'preview_type' => 'search',
				],
		...
		$this->add_control(
			...
			[
				...
				'label' => __( 'Apply & Preview', 'madxartwork-pro' ),
				'label_block' => true,
				'show_label' => false,
				'text' => __( 'Apply & Preview', 'madxartwork-pro' ),
				'separator' => 'none',
				'event' => 'madxartworkThemeBuilder:ApplyPreview',
			]
		);
	...
	public function get_wp_preview_url() {
		 ...
		 */
		$preview_url = apply_filters( 'madxartwork/document/wp_preview_url', $preview_url, $this );
