...
class Archive extends Theme_Page_Document {
	...
	public static function get_title() {
		return __( 'Archive', 'madxartwork-pro' );
	}
	...
	public static function get_preview_as_options() {
		...
		foreach ( $post_types as $post_type => $label ) {
			...
			if ( $post_type_object->has_archive ) {
				$post_type_archives[ 'post_type_archive/' . $post_type ] = sprintf( __( '%s Archive', 'madxartwork-pro' ), $post_type_object->label );
			}
			...
			foreach ( $post_type_taxonomies as $slug => $object ) {
				$taxonomies[ 'taxonomy/' . $slug ] = sprintf( __( '%s Archive', 'madxartwork-pro' ), $object->label );
			}
		}

		$options = [
			'archive/recent_posts' => __( 'Recent Posts', 'madxartwork-pro' ),
			'archive/date' => __( 'Date Archive', 'madxartwork-pro' ),
			'archive/author' => __( 'Author Archive', 'madxartwork-pro' ),
			'search' => __( 'Search Results', 'madxartwork-pro' ),
		];
		...
		return [
			'archive' => [
				'label' => __( 'Archive', 'madxartwork-pro' ),
				'options' => $options,
			],
		];
	}
