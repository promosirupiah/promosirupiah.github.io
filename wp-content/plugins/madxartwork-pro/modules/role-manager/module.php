<?php
namespace madxartworkPro\Modules\RoleManager;

use madxartworkPro\Plugin;
use madxartworkPro\Base\Module_Base;
use madxartwork\Core\RoleManager\Role_Manager as RoleManagerBase;
...
class Module extends Module_Base {
	...
	public function display_role_controls( $role_slug, $role_data ) {
		...
		if ( ! $options ) {
			$options = [
				'excluded_options' => Plugin::madxartwork()->role_manager->get_role_manager_options(),
				'advanced_options' => $this->get_role_manager_options(),
			];
		}
		$id = self::ROLE_MANAGER_OPTION_NAME . '_' . $role_slug . '_design';
		$name = 'madxartwork_' . self::ROLE_MANAGER_OPTION_NAME . '[' . $role_slug . '][]';
		...
		<label for="<?php echo esc_attr( $id ); ?>">
			...
			<?php esc_html_e( 'Access to edit content only', 'madxartwork-pro' ); ?>
		</label>
		<?php
	}
	...
	public function __construct() {
		parent::__construct();
		if ( is_admin() ) {
			add_action( 'madxartwork/admin/after_create_settings/' . RoleManagerBase::PAGE_ID, [ $this, 'register_admin_fields' ], 100 );
		}
		remove_action( 'madxartwork/role/restrictions/controls', [ Plugin::madxartwork()->role_manager, 'get_go_pro_link_html' ] );
		add_action( 'madxartwork/role/restrictions/controls', [ $this, 'display_role_controls' ], 10, 2 );
		add_filter( 'madxartwork/editor/user/restrictions', [ $this, 'get_user_restrictions' ] );
	}
}
