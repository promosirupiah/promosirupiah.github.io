...
class Slack extends Action_Base {
	...
	public function get_label() {
		return __( 'Slack', 'madxartwork-pro' );
	}
	...
	public function register_settings_section( $widget ) {
		$widget->start_controls_section(
			'section_slack',
			[
				'label' => __( 'Slack', 'madxartwork-pro' ),
				'condition' => [
		...
		$widget->add_control(
			...
			[
				...
				'label_block' => true,
				'separator' => 'before',
				'description' => __( 'Enter the webhook URL that will receive the form\'s submitted data.', 'madxartwork-pro' ) . ' ' . sprintf( '<a href="%s" target="_blank">%s</a>.', 'https://slack.com/apps/A0F7XDUAZ-incoming-webhooks/', __( 'Click here for Instructions', 'madxartwork-pro' ) ),
				'render_type' => 'none',
		...
		$widget->add_control(
			'slack_channel',
			[
				'label' => __( 'Channel', 'madxartwork-pro' ),
				'type' => Controls_Manager::TEXT,
			]
		);
		...
		$widget->add_control(
			'slack_pretext',
			[
				'label' => __( 'Pre Text', 'madxartwork-pro' ),
				'type' => Controls_Manager::TEXT,
			]
		);
	...
	public function run( $record, $ajax_handler ) {
		...
		$attachment = [
			'text' => __( 'A new Form Submission has been received', 'madxartwork-pro' ),
			'title' => __( 'A new Submission', 'madxartwork-pro' ),
		...
		if ( ! empty( $settings['slack_add_ts'] ) && 'yes' === $settings['slack_add_ts'] ) {
			$attachment = array_merge( $attachment, [
				'footer' => sprintf( __( 'Powered by %s', 'madxartwork-pro' ), 'madxartwork' ),
				'footer_icon' => is_ssl() ? madxartwork_ASSETS_URL . 'images/logo-icon.png' : null,
				'ts' => time(),
			] );
		}
		...
		$webhook_data = apply_filters( 'madxartwork_pro/forms/slack/webhook_args', $webhook_data );
