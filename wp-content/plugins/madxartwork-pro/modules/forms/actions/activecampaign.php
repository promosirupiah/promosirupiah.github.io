...
class Activecampaign extends Classes\Integration_Base {
	...
	private function get_global_api_url() {
		return get_option( 'madxartwork_' . self::OPTION_NAME_API_URL, '' );
	}
	...
	public function get_label() {
		return __( 'ActiveCampaign', 'madxartwork-pro' );
	}
	...
	public function register_settings_section( $widget ) {
		$widget->start_controls_section(
			...
			[
				'label' => __( 'ActiveCampaign', 'madxartwork-pro' ),
				'condition' => [
		...
		$widget->add_control(
			...
			[
				'label' => __( 'API Credentials', 'madxartwork-pro' ),
		...
		$widget->add_control(
			...
			[
				'label' => __( 'API URL', 'madxartwork-pro' ),
				'type' => Controls_Manager::TEXT,
				'description' => __( 'Use this field to set a custom API URL for the current form', 'madxartwork-pro' ),
				'condition' => [
					'activecampaign_api_credentials_source' => 'custom',
				],
			]
	...
	public function run( $record, $ajax_handler ) {
		...
		if ( ! $subscriber ) {
			$ajax_handler->add_admin_error_message( __( 'ActiveCampaign Integration requires an email field and a selected list', 'madxartwork-pro' ) );
	...
	public function register_admin_fields( Settings $settings ) {
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'activecampign', [
			'callback' => function() {
				echo '<hr><h2>' . esc_html__( 'ActiveCampaign', 'madxartwork-pro' ) . '</h2>';
			},
			'fields' => [
				...
				self::OPTION_NAME_API_URL => [
					...
					'field_args' => [
						'type' => 'url',
						'desc' => sprintf( __( 'To integrate with our forms you need an <a href="%s" target="_blank">API Key</a>.', 'madxartwork-pro' ), 'https://help.activecampaign.com/hc/en-us/articles/207317590-Getting-started-with-the-API' ),
					],
				],
				'validate_api_data' => [
					'field_args' => [
						'type' => 'raw_html',
						'html' => sprintf( '<button data-action="%s" data-nonce="%s" class="button madxartwork-button-spinner" id="madxartwork_pro_activecampaign_api_key_button">%s</button>', self::OPTION_NAME_API_KEY . '_validate', wp_create_nonce( self::OPTION_NAME_API_KEY ), __( 'Validate API Key', 'madxartwork-pro' ) ),
					],
				],
			],
		] );
