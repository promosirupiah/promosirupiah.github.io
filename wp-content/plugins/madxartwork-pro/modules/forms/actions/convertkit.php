...
use madxartworkPro\Modules\Forms\Classes\Convertkit_Handler;
...
class Convertkit extends Integration_Base {
	...
	public function get_label() {
		return __( 'ConvertKit', 'madxartwork-pro' );
	}
	...
	public function register_settings_section( $widget ) {
		$widget->start_controls_section(
			'section_convertkit',
			[
				'label' => __( 'ConvertKit', 'madxartwork-pro' ),
				'condition' => [
	...
	public function run( $record, $ajax_handler ) {
		...
		if ( ! $subscriber ) {
			$ajax_handler->add_admin_error_message( __( 'ConvertKit Integration requires an email field', 'madxartwork-pro' ) );
	...
	public function register_admin_fields( Settings $settings ) {
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'convertkit', [
			'callback' => function() {
				echo '<hr><h2>' . esc_html__( 'ConvertKit', 'madxartwork-pro' ) . '</h2>';
			},
			'fields' => [
				self::OPTION_NAME_API_KEY => [
					...
					'field_args' => [
						'type' => 'text',
						'desc' => sprintf( __( 'To integrate with our forms you need an <a href="%s" target="_blank">API Key</a>.', 'madxartwork-pro' ), 'https://app.convertkit.com/account/edit' ),
					],
				],
				'validate_api_data' => [
					'field_args' => [
						'type' => 'raw_html',
						'html' => sprintf( '<button data-action="%s" data-nonce="%s" class="button madxartwork-button-spinner" id="madxartwork_pro_convertkit_api_key_button">%s</button>', self::OPTION_NAME_API_KEY . '_validate', wp_create_nonce( self::OPTION_NAME_API_KEY ), __( 'Validate API Key', 'madxartwork-pro' ) ),
					],
				],
			],
		] );
