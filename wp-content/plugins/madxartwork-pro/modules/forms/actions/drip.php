...
use madxartworkPro\Modules\Forms\Classes\Drip_Handler;
...
class Drip extends Integration_Base {
	...
	public function get_label() {
		return __( 'Drip', 'madxartwork-pro' );
	}
	...
	public function register_settings_section( $widget ) {
		...
		$widget->add_control(
			...
			[
				'label' => __( 'API Token', 'madxartwork-pro' ),
		...
		$widget->add_control(
			...
			[
				'label' => __( 'Custom API Token', 'madxartwork-pro' ),
				'type' => Controls_Manager::TEXT,
				'condition' => [
					'drip_api_token_source' => 'custom',
				],
				'description' => __( 'Use this field to set a custom API token for the current form', 'madxartwork-pro' ),
			]
		);
		...
		$widget->add_control(
			'drip_account',
			[
				'label' => __( 'Account', 'madxartwork-pro' ),
				'type' => Controls_Manager::SELECT,
				'options' => [],
				'render_type' => 'none',
				'conditions' => [
		...
		$widget->add_control(
			...
			[
				'label' => __( 'Send Additional Data to Drip', 'madxartwork-pro' ),
		...
		$widget->add_control(
			...
			[
				...
				'default' => 'no',
				'description' => __( 'Send all form fields to drip as custom fields', 'madxartwork-pro' ),
				'condition' => [
					'drip_account!' => '',
				],
			]
		...
		$widget->add_control(
			...
			[
				...
				'type' => Controls_Manager::TEXT,
				'description' => __( 'Add as many tags as you want, comma separated.', 'madxartwork-pro' ),
				'condition' => [
					'drip_account!' => '',
				],
			]
	...
	public function run( $record, $ajax_handler ) {
		...
		if ( ! $subscriber ) {
			$ajax_handler->add_admin_error_message( __( 'Drip Integration requires an email field', 'madxartwork-pro' ) );
	...
	private function get_meta_data( $meta_data ) {
		...
		foreach ( $meta_data as $meta_type ) {
			switch ( $meta_type ) {
				...
				case 'credit':
					$custom_fields[ $meta_type ] = sprintf( __( 'Powered by %s', 'madxartwork-pro' ), 'https://madxartwork.com/pro/' );
					break;
			}
		}
	...
	public function register_admin_fields( Settings $settings ) {
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'drip', [
			'callback' => function() {
				echo '<hr><h2>' . esc_html__( 'Drip', 'madxartwork-pro' ) . '</h2>';
			},
			'fields' => [
				self::OPTION_NAME_API_KEY => [
					...
					'field_args' => [
						'type' => 'text',
						'desc' => sprintf( __( 'To integrate with our forms you need an <a href="%s" target="_blank">API Key</a>.', 'madxartwork-pro' ), 'http://kb.getdrip.com/general/where-can-i-find-my-api-token/' ),
					],
				],
				'validate_api_data' => [
					'field_args' => [
						'type' => 'raw_html',
						'html' => sprintf( '<button data-action="%s" data-nonce="%s" class="button madxartwork-button-spinner" id="madxartwork_pro_drip_api_token_button">%s</button>', self::OPTION_NAME_API_KEY . '_validate', wp_create_nonce( self::OPTION_NAME_API_KEY ), __( 'Validate API Token', 'madxartwork-pro' ) ),
					],
				],
			],
		] );
