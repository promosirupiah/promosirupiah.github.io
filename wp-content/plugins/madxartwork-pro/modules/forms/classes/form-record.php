...
class Form_Record {
	...
	public function get_formatted_data( $with_meta = false ) {
		$formatted = [];
		$no_label = __( 'No Label', 'madxartwork-pro' );
		$fields = $this->fields;
	...
	public function validate( $ajax_handler ) {
		foreach ( $this->fields as $id => $field ) {
			...
			/**
			 * madxartwork form field validation.
			 *
			 ...
			 * @param Ajax_Handler $ajax_handler An instance of the ajax handler.
			 */
			do_action( "madxartwork_pro/forms/validation/{$field_type}", $field, $this, $ajax_handler );
		}

		/**
		 * madxartwork form validation.
		 *
		 ...
		 * @param Ajax_Handler $ajax_handler An instance of the ajax handler.
		 */
		do_action( 'madxartwork_pro/forms/validation', $this, $ajax_handler );
	...
	public function process_fields( $ajax_handler ) {
		foreach ( $this->fields as $id => $field ) {
			...
			/**
			 * madxartwork form field process.
			 *
			 ...
			 * @param Ajax_Handler $ajax_handler An instance of the ajax handler.
			 */
			do_action( "madxartwork_pro/forms/process/{$field_type}", $field, $this, $ajax_handler );
		}

		/**
		 * madxartwork form process.
		 *
		 ...
		 * @param Ajax_Handler $ajax_handler An instance of the ajax handler.
		 */
		do_action( 'madxartwork_pro/forms/process', $this, $ajax_handler );
	}
	...
	private function set_meta() {
		...
		foreach ( $form_metadata as $metadata_type ) {
			switch ( $metadata_type ) {
				...
				case 'page_url':
					$this->meta['page_url'] = [
						'title' => __( 'Page URL', 'madxartwork-pro' ),
				...
				case 'user_agent':
					$this->meta['user_agent'] = [
						'title' => __( 'User Agent', 'madxartwork-pro' ),
				...
				case 'remote_ip':
					$this->meta['remote_ip'] = [
						'title' => __( 'Remote IP', 'madxartwork-pro' ),
						'value' => Utils::get_client_ip(),
					];
					break;
				case 'credit':
					$this->meta['credit'] = [
						'title' => __( 'Powered by', 'madxartwork-pro' ),
						'value' => __( 'madxartwork', 'madxartwork-pro' ),
					];
					break;
			}
		}
	...
	private function sanitize_field( $field, $value ) {
		...
		switch ( $field_type ) {
			...
			default:
				 ...
				 */
				$value = apply_filters( "madxartwork_pro/forms/sanitize/{$field_type}", $value, $field );
		}
