...
abstract class Form_Base extends Base_Widget {
	...
	public function on_export( $element ) {
		/** @var \madxartworkPro\Modules\Forms\Classes\Action_Base[] $actions */
	...
	protected function make_textarea_field( $item, $item_index ) {
		$this->add_render_attribute( 'textarea' . $item_index, [
			'class' => [
				...
				'madxartwork-size-' . $item['input_size'],
			],
	...
	protected function make_radio_checkbox_field( $item, $item_index, $type ) {
		...
		$html = '';
		if ( $options ) {
			$html .= '<div class="madxartwork-field-subgroup ' . esc_attr( $item['css_classes'] ) . ' ' . $item['inline_list'] . '">';
			foreach ( $options as $key => $option ) {
				...
				$html .= '<span class="madxartwork-field-option"><input ' . $this->get_render_attribute_string( $element_id ) . '> <label for="' . $html_id . '">' . $option_label . '</label></span>';
			}
			$html .= '</div>';
		}
	...
	protected function form_fields_render_attributes( $i, $instance, $item ) {
		$this->add_render_attribute(
			[
				'field-group' . $i => [
					'class' => [
						'madxartwork-field-type-' . $item['field_type'],
						'madxartwork-field-group',
						'madxartwork-column',
						'madxartwork-field-group-' . $item['custom_id'],
					],
				],
				'input' . $i => [
		...
		$this->add_render_attribute( 'field-group' . $i, 'class', 'madxartwork-col-' . $item['width'] );

		if ( ! empty( $item['width_tablet'] ) ) {
			$this->add_render_attribute( 'field-group' . $i, 'class', 'madxartwork-md-' . $item['width_tablet'] );
		}

		if ( $item['allow_multiple'] ) {
			$this->add_render_attribute( 'field-group' . $i, 'class', 'madxartwork-field-type-' . $item['field_type'] . '-multiple' );
		}

		if ( ! empty( $item['width_mobile'] ) ) {
			$this->add_render_attribute( 'field-group' . $i, 'class', 'madxartwork-sm-' . $item['width_mobile'] );
		}
		...
		if ( ! $instance['show_labels'] ) {
			$this->add_render_attribute( 'label' . $i, 'class', 'madxartwork-screen-only' );
		}

		if ( ! empty( $item['required'] ) ) {
			$class = 'madxartwork-field-required';
			if ( ! empty( $instance['mark_required'] ) ) {
				$class .= ' madxartwork-mark-required';
			}
